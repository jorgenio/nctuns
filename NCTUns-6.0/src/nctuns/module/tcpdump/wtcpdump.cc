...
MODULE_GENERATOR(wtcpdump);


wtcpdump::wtcpdump(u_int32_t type, u_int32_t id, struct plist* pl, const char *name)
...
void wtcpdump::modifypkt(Packet *pkt) {
	...
	if (pktflow == SEND_FLOW) {
		/* tunID or choose a tunnel for wtcpdump if the module
...
void wtcpdump::pktdump(Packet *pkt) {
...
int wtcpdump::recv(ePacket_ *pkt) {
	ePacket_		*pkt_;
	Packet                  *p;
	...
	if( dump_flag == 1 ) {
		pkt_ = pkt_copy(pkt);
		p = (Packet *)pkt_->DataInfo_;
		mac_hdr = (struct hdr_mac802_11 *)p->pkt_get();

		/* decapsulate the 802.11 mac header */
		if( mac_hdr->dh_fc.fc_type == MAC_Type_Data ) {
...
int wtcpdump::send(ePacket_ *pkt) {
	ePacket_		*pkt_;
	Packet			*p;
	struct hdr_mac802_11	*mac_hdr;
	...
	tunID = GET_REG_VAR1(get_portls(), "TUNID", u_long *);
 	...
 	if (dumptunfd < 0) {
		dumptunfd = tun_alloc("tun0");
 		if (dumptunfd < 0) {
 			printf("Error: wtcpdump %d: open tunnel tun0 error\n",
					get_nid());
			exit(-1);
		}
        }
...
int wtcpdump::command(int argc, const char *argv[]) {

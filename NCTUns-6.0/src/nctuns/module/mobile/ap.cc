...
AP::AP(u_int32_t type, u_int32_t id, struct plist* pl, const char *name)
...
int AP::init() {
	...
	type = POINTER_TO_MEMBER(AP,GenerateBeacon);	
	...
	if( multicast_flag == 0 ) {
		...
		if(mac_hdr->dh_fc.fc_type == MAC_Type_Management) {
			if(mac_hdr->dh_fc.fc_subtype == MAC_Subtype_Probe_Req) {
				...
				printf("AP %d : Receive the undefined mac802_11 management pkt\n"
					,get_nid());
				freePacket(pkt);
				return(1);
			}
			...
			printf("AP %d : Receive the undefined mac802_11 packet !!\n",get_nid());
			freePacket(pkt);
			return(1);
		}
	}
...
int AP::send(ePacket_ *pkt) {
	...
	if( ap_module_flag == 0 ) {
		if(sendtarget_->get_curqlen() > 0 || DataBuf_len > 0 || ManageBuf_len > 0)
			...
			upcall = (int (NslObject::*)(MBinder *))&AP::PushBufferPkt;
		...
		if(return_ep != NULL) {
			if(GET_MLEVEL(return_ep) <= MLEVEL_2) {
				/* A data packet. */
				if(DataBuf_len < DATA_BUF_LENGTH) {
				...
				if(ManageBuf_len < MANAGE_BUF_LENGTH) {
					...
					printf("AP %d : lose management pkt !!\n",get_nid());
					freePacket(return_ep);
				}
			}
...
int AP::command(int argc, const char *argv[]) {
...
void AP::FlushAssoTable(){
	...
	for(tmp_entry = AssoTable; tmp_entry != NULL; 
		...
		if(cur_time - tmp_entry->update_time > period) {
			...
			multicast_type = POINTER_TO_MEMBER(AP,recv);
	...
	if( AssoCount != 0){
		BASE_OBJTYPE(type);
		type = POINTER_TO_MEMBER(AP,FlushAssoTable);
...
void AP::GenerateBeacon(){
	...
	mac_hdr = (struct hdr_mac802_11 *)
		beacon_pkt->pkt_malloc(sizeof(struct hdr_mac802_11));
	...
	timestamp = (unsigned long long)GetNodeCurrentTime(get_nid());
	SetTimestampInto80211BeaconFrameBody(framebody, &timestamp); 

	BeaconInterval = (unsigned short)BEACON_TIMEVAL;
	...
	BASE_OBJTYPE(type);
	type = POINTER_TO_MEMBER(AP,send);
	ep = createEvent();
	...
	BASE_OBJTYPE(type_);
	type_ = POINTER_TO_MEMBER(AP,GenerateBeacon);	
...
void AP::ProcessChanlScanReq(ePacket_ *pkt){
	...
	mac_hdr = (struct hdr_mac802_11 *)
		probe_resp_pkt->pkt_malloc(sizeof(struct hdr_mac802_11));	
...
void AP::ProcessAsso(ePacket_ *pkt){
	...
	nodeID = 0;
	SSID_Len = 0;
	tmp_ptr = GetSSIDFrom80211AssociationRequestFrameBody(asso_req_framebody, &SSID_Len);
	...
	if(AssoTable == NULL){
		...
		for(tmp_entry = AssoTable; tmp_entry != NULL; tmp_entry =
						tmp_entry->next_entry){
			if(tmp_entry->nodeID == (unsigned int)nodeID){
				tmp_entry->update_time = GetNodeCurrentTime(get_nid());
				find_flag = 1;
				break;
			}
...

	{
		...
		up_pkt = new Packet;
		up_pkt->pkt_setflow(PF_RECV);
		eh = (struct ether_header*)up_pkt->pkt_malloc(sizeof(struct ether_header) + 1);
		eh->ether_type = 1;
		memcpy(eh->ether_dhost, "\0\0\0\0\0\0",6);
		memcpy(eh->ether_shost,(char *)MAC_HDR->dh_addr2,6);
		up_ep = createEvent();
		up_ep->DataInfo_ = up_pkt;
		p = ((char*)eh)+sizeof(struct ether_header);
		*p = 1;
		//printf("%s get associate ether_shot %d\n", get_name(), eh->ether_shost[5]);
		put(up_ep, recvtarget_);
	}
	...
	asso_resp_pkt->pkt_addinfo("mflag", tmp_buf, 1);	
        ...
        mac_hdr = (struct hdr_mac802_11 *)
		asso_resp_pkt->pkt_malloc(sizeof(struct hdr_mac802_11));
	...
	memcpy((char *)mac_hdr->dh_addr3,(char *)mac_,6);
         ...
         */
	SupportedRates_Len = 1;
	framebody_len = Calculate80211AssociationResponseFrameBodyLength(SupportedRates_Len);
	...
	InitializeFrameBodyWithZero(asso_resp_framebody, framebody_len);

	AID = (unsigned short)get_nid();
	SetAIDInto80211AssociationResponseFrameBody(asso_resp_framebody, &AID);
	...
	/*	
		...
		broadcast_type = POINTER_TO_MEMBER(AP,recv);
...
void AP::ProcessReAsso(ePacket_ *pkt){
	...
	nodeID = 0;
	SSID_Len = 0;
	tmp_ptr = GetSSIDFrom80211ReassociationRequestFrameBody(reasso_req_framebody, &SSID_Len);
	if(tmp_ptr != NULL && SSID_Len != 0) 
		bcopy(tmp_ptr, (void *)&nodeID, SSID_Len);

        /* update association table */
	if(AssoCount == 0){
		printf("Warning : AP %d receive an unexpected reasso request.\n"
			,get_nid());
		printf("Warning : AP %d put this request to asso procedure.\n"
			,get_nid());
		ProcessAsso(pkt);
		return;
	}
	...
	reasso_resp_pkt->pkt_addinfo("mflag", tmp_buf, 1);	
        ...
        mac_hdr = (struct hdr_mac802_11 *)
		reasso_resp_pkt->pkt_malloc(sizeof(struct hdr_mac802_11));
	...
	memcpy((char *)mac_hdr->dh_addr3,(char *)mac_,6);
        ...
        /*
         * Fill the frame body of reassociation response frame.
         */
	SupportedRates_Len = 1;
	...
	InitializeFrameBodyWithZero(reasso_resp_framebody, framebody_len);

	AID = (unsigned short)get_nid();
	SetAIDInto80211ReassociationResponseFrameBody(reasso_resp_framebody, &AID);
...
void AP::ProcessDisAsso(ePacket_ *pkt){

	Packet			*pkt_;
	hdr_mac802_11		*mac_hdr;
	struct AssoEntry	*tmp_entry;

	hdr_mac802_11		*disasso_mac_hdr;
	Packet			*disasso_pkt;
	Event_			*disasso_ep;
	...
	for(tmp_entry = AssoTable; tmp_entry != NULL; 
		...
		if( !bcmp(mac_hdr->dh_addr2, tmp_entry->mac_addr, 6) ) {
			...
			disasso_mac_hdr = (struct hdr_mac802_11 *)
			...
			disasso_type = POINTER_TO_MEMBER(AP,send);
...
int AP::CheckAssoAddr(char *addr, Packet *pkt_){
	struct AssoEntry	*tmp_entry;
...
void AP::ProcessRecvBroPkt(ePacket_ *pkt){
	ePacket_			*epkt;
	Packet				*pkt_;

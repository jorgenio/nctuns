...
#include <mobile/qosMN.h>
#include <mbinder.h>

MODULE_GENERATOR(qosMN);

char	broadcast_addr[6]={0xff,0xff,0xff,0xff,0xff,0xff};

qosMN::qosMN(u_int32_t type, u_int32_t id, struct plist* pl, const char *name)
...
int qosMN::init() {
	...
	if(comm_status == AD_HOC){
		...
		*using_chanl = 0;
		BASE_OBJTYPE(type);
		type = POINTER_TO_MEMBER(qosMN,ActiveChanlScan);
		...
		*using_chanl = 0;
		BASE_OBJTYPE(type);
		type = POINTER_TO_MEMBER(qosMN,PassiveChanlScan);
...
int qosMN::recv(ePacket_ *pkt) {
	...
	if(mac_hdr->dh_fc.fc_type == MAC_Type_Management){
		...
		//	BASE_OBJTYPE(type);
		//	type = POINTER_TO_MEMBER(qosMN,ReAssociation);
		...
		//	ReAssoTimer.start(period,0);
			...
			printf("qosMN %d : Receive the undefined mac802_11 management pkt\n"
				,get_nid());
			freePacket(pkt);
			return(1);
		}
		...
		printf("qosMN %d : Receive the undefined mac802_11 packet !!\n",get_nid());
		freePacket(pkt);
		return(1);
	}
}
...
int qosMN::send(ePacket_ *pkt) {
	...
	if( mobile_module_flag == 0 ) {
		if( sendtarget_->get_curqlen() > 0 || DataBuf_len > 0 
			...
			if(!bcmp(eh->ether_dhost,broadcast_addr,6)){
			...
			upcall = (int (NslObject::*)(MBinder *))&qosMN::PushBufferPkt;
		...
		if(return_ep != NULL) {
			if(GET_MLEVEL(return_ep) <= MLEVEL_2) {
				...
				if(ManageBuf_len < MANAGE_BUF_LENGTH) {
					...
					printf("qosMN %d : Lose management pkt !!\n",get_nid());
...
void qosMN::ActiveChanlScan(){
	...
	mac_hdr = (struct hdr_mac802_11e_manage *)
			probe_pkt->pkt_malloc(sizeof(struct hdr_mac802_11e_manage));
	...
	if(comm_status == INFRASTRUCTURE && scan_mode == ACTIVE_SCAN){
		if(++(*using_chanl) > CHANNEL_NUM){
			...
			type = POINTER_TO_MEMBER(qosMN,send);
			ep = createEvent();
...
void qosMN::FindAccessPoint(ePacket_ *pkt){
	...
	SSID_Len = 0;
	tmp_ptr = GetSSIDFrom80211eProbeResponseFrameBody(framebody, &SSID_Len);
...
void qosMN::Association(){
	...
	mac_hdr = (struct hdr_mac802_11e_manage *)
			asso_pkt->pkt_malloc(sizeof(struct hdr_mac802_11e_manage));
	...
	type = POINTER_TO_MEMBER(qosMN,RetryAssociation);
	RetryAssoTimer.setCallOutObj(this, type);
	RetryAssoTimer.start(timeval,0);
}

void qosMN::RetryAssociation() {
	if(action == ASSO) Association();
}

void qosMN::ReAssociation(){
...
//	action = REASSO;
	...
	mac_hdr = (struct hdr_mac802_11e_manage *)
			reasso_pkt->pkt_malloc(sizeof(struct hdr_mac802_11e_manage));
	...
	SSID_Len = 4;
	SupportedRates_Len = 1;
	framebody_len = Calculate80211eReassociationRequestFrameBodyLength(SSID_Len, SupportedRates_Len);
	...
	SetCurrentAPAddressInto80211eReassociationRequestFrameBody(framebody, (void *)CurrentAPAddress);

	SSID = (unsigned int)get_nid();
	SetSSIDInto80211eReassociationRequestFrameBody(framebody, SSID_Len, &SSID);
...
void qosMN::ProcessBeacon(ePacket_ *pkt){
	Packet				*pkt_;
	struct hdr_mac802_11e_manage	*mac_hdr;
	...
	SSID_Len = 0;
	tmp_ptr = GetSSIDFrom80211eBeaconFrameBody(framebody, &SSID_Len);
	if(tmp_ptr != NULL && SSID_Len != 0) 
		bcopy(tmp_ptr, (void *)&SSID, SSID_Len);
	...
	if( bcmp(active_ap->ap_mac_addr, mac_hdr->dh_addr3, 6) &&
	...
	if( action == DATA_FLOW){
		...
		if( checkAPconn_flag == 0){
			...
			BASE_OBJTYPE(type);
			type = POINTER_TO_MEMBER(qosMN,CheckApConnectivity);
...
void qosMN::ProcessBeaconDuringPassiveScan(ePacket_ *pkt) {
	FindAccessPoint(pkt);
}

void qosMN::PushBufferPkt(){
	ePacket_		*pkt;
	int			(NslObject::*upcall)(MBinder *);
...
void qosMN::CheckApConnectivity(){

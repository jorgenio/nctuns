...
#include <module/opt/FIFO/ofifo.h>
#include <exportStr.h>
#include <mbinder.h>
...
MODULE_GENERATOR(ofifo);

ofifo::ofifo(u_int32_t type, u_int32_t id, struct plist* pl, const char *name)
...
int ofifo::init() {
	...
	if ( (log_qlen_flag)&&(!strcasecmp(log_qlen_flag, "on")) ){
		...
		if ( (log_option)&&(!strcasecmp(log_option, "FullLog")) ){
			...
			BASE_OBJTYPE(type);
			type = POINTER_TO_MEMBER(ofifo, log);
	...
	/* set upcall */
        upcall = (int (NslObject::*)(MBinder *))&ofifo::intrq;
...
int ofifo::send(ePacket_ *pkt) {
	...
	if( sendtarget_->qfull() ) {
		...
		if (mytunidp != NULL) {
			...
			if (0) printf("enqueue: FIFO(nid %d) pid %d tunid %lu fifoIFcurqlen[%d]\n", get_nid(), get_port(), *mytunidp, fifoIFcurqlen[*mytunidp]);
		}
...
int ofifo::intrq(MBinder *port) {
	...
	if (mytunidp != NULL) {
		...
		// associated with a tunnel interface.
		fifoIFcurqlen[*mytunidp] = if_snd.ifq_len;
		if (0) printf("dequeue: FIFO(nid %d) pid %d tunid %lu fifoIFcurqlen[%d]\n", get_nid(), get_port(), *mytunidp, fifoIFcurqlen[*mytunidp]);
	}
...
int ofifo::command(int argc, const char *argv[]) {
	char			tmpBuf[10];

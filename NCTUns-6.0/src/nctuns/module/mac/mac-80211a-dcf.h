...
#ifndef __NCTUNS_mac_802_11a_dcf_h__
#define __NCTUNS_mac_802_11a_dcf_h__
...
/*for cross-module function call,hychen added*/
#include <mbinder.h>
#include <phy/80211a_phy/ofdm_80211a.h>
...
/*
 * Note XXX:
 *	- Time unit here is microsecond. hychen modified since 2008/06/04
 */
#define PRM_Time (u_int32_t)(16+4)		/* 80211a channel spacing = 20MHz*/
#define RTS_Time(nid,pid)	 						\
	(u_int32_t)(rintf(((phy_80211a*)InstanceLookup(nid,pid,"phy_80211a"))->TXTIME(MAC80211_RTS_LEN,*bw_)+0.5))
#define CTS_Time(nid,pid)							\
	(u_int32_t)(rintf(((phy_80211a*)InstanceLookup(nid,pid,"phy_80211a"))->TXTIME(MAC80211_CTS_LEN,*bw_)+0.5))
#define ACK_Time(nid,pid)						\
	(u_int32_t)(rintf(((phy_80211a*)InstanceLookup(nid,pid,"phy_80211a"))->TXTIME(MAC80211_ACK_LEN,*bw_)+0.5))
#define TX_Time(len,bw,nid,pid)							\
	(u_int32_t)(rintf(((phy_80211a*)InstanceLookup(nid,pid,"phy_80211a"))->TXTIME(len,bw)+0.5))
...
#define ACKTimeout(len,nid,pid)     (TX_Time(len,(*bw_),nid,pid) + ACK_Time(nid,pid) + sifs+difs)
...
/*========================================================================
   Class IEEE 802.11a DCF
  ========================================================================*/
class mac802_11a_dcf : public NslObject {
 ...
 private :
	...
	unsigned int		crc32(unsigned int,char*,int);

 public :
 
	mac802_11a_dcf(u_int32_t type, u_int32_t id, struct plist* pl, const char *name);
	~mac802_11a_dcf();

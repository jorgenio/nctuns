...
class ChannelModel {
	...
	double _txHeight;	// in meter
	double _rxHeight;	// in meter
	...
	double SNRToENR(double SNR, double dataRate);
      ...
      public:
	 ChannelModel(double frequency, double txHeight, double rxHeight,
		      double bandwidth, double symbolDuration,
		      bool isEnabled);
	~ChannelModel();

	double receivePower(double transPower, double dist);
	double powerToSNR(double power);
	double computeSNR(double transPower, double dist);

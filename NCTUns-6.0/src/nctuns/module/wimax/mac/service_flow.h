...
#ifndef __NCTUNS_serviceflow_h__
#define __NCTUNS_serviceflow_h__
...
class ServiceFlow {
    ...
    ServiceFlow() {
        ...
        int i, imac[6];
        sscanf(mac, "%x:%x:%x:%x:%x:%x", imac, imac + 1, imac + 2,
    ...
    void Dump() {
        ...
        printf("ssmac: %02x:%02x:%02x:%02x:%02x:%02x\n", ssmac[0],
            ssmac[1], ssmac[2], ssmac[3], ssmac[4], ssmac[5]);
        printf("direction: %c\n", direction);
        printf("qosindex: %d\n", qosindex);
        printf("sfstate: %d\n", sfstate);
...
class ClassifierRule {
    ...
    int Match(struct ip *iphdr);
    int MatchDst(u_int32_t);
...
class ProvisionedSfTable {
    std::vector<ServiceFlow*> List;

    public:
    void AddItem(ServiceFlow * sf) {
        List.push_back(sf);
    }

    ServiceFlow *GetServiceFlow(u_int32_t sfindex) {
    ...
    ServiceFlow *GetServiceFlow(u_char mac[6]) {
        std::vector<ServiceFlow*>::iterator iter;
...
class ServiceClassTable {
    ...
    std::vector < ServiceClass * >sclist;
    public:

    void AddItem(ServiceClass * sc) {
        sclist.push_back(sc);
    }

    ServiceClass *GetServiceClass(u_int32_t qosIndex) {
        std::vector < ServiceClass * >::iterator iter;
...
private: 
    std::vector<ClassifierRule*> List;
    
public:
    explicit ClassifierRuleTable();
    virtual ~ClassifierRuleTable();

    ClassifierRule* Find(ether_header*, ip*);
    ClassifierRule* Find(ip *iphdr);
    ClassifierRule* Find(uint32_t);
    void Dump();

    inline void AddItem(ClassifierRule * rl) { List.push_back(rl); } 
    inline uint32_t get_table_size() { return List.size(); }
};

class TableReader {
    public:
    int parseLine(char **argv, int size, char *inputstr, const char *sep) {
        int i = 0;
        while (i < size) {
...
class SfTableReader:TableReader {
    public:
    SfTableReader(FILE * fp, ProvisionedSfTable * sftable);
};

class SfClassTableReader:TableReader {
    public:
    SfClassTableReader(FILE * fp, ServiceClassTable * sctable);
};

class ClassifierRuleTableReader:TableReader {
...
#endif				// __NCTUNS_serviceflow_h__

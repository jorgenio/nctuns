...
/*
 * Member function definitions of class `Mac_address'.
 */
...
void
Mac_address::copy_from(uint8_t* src_addr, size_t len)
{
    if (len > ADDRESS_LEN)
        len = ADDRESS_LEN;
...
void
Mac_address::copy_to(uint8_t* dst_addr, size_t len)
{
    if (len > ADDRESS_LEN)
        len = ADDRESS_LEN;
...
#if 0
int
Mac_address::operator ==(uint8_t addr[6])
{
    for (int i = 0; i < 6; i++) {
        if (maca[i] < macb[i])
            return -1;	// <

...
#ifndef __NCTUNS_80216J_OFDMA_PMPRS_MR_H__
#define __NCTUNS_80216J_OFDMA_PMPRS_MR_H__
...
class OFDMA_PMPRS_MR:public OFDMA_80216j {
	private:
		timerObj                *_recvTimer;
		...
		bool			trans_flag;	// indicate if transparent existed
		...
		void    recvHandler_subordinate	(Event *);
		int     procCmdFromMAC  (Pkthdr *);
		int     procRNGCode     (Pkthdr *);
		void    decodeDLFP      (char *, struct DLFP_except_128 *);

	public:
		OFDMA_PMPRS_MR(uint32_t type, uint32_t id, struct plist *pl, const char *name);
		~OFDMA_PMPRS_MR();
...
#endif                /* __NCTUNS_80216J_OFDMA_PMPRS_MR_H__ */

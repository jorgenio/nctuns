...
#include "ofdma_pmpbs_MR.h"

MODULE_GENERATOR(OFDMA_PMPBS_MR);
...
extern char *MobileRelayWIMAXChannelCoding;
	...
	OFDMA_PMPBS_MR::OFDMA_PMPBS_MR(uint32_t type, uint32_t id, struct plist *pl, const char *name)
...
OFDMA_PMPBS_MR::~OFDMA_PMPBS_MR()
{
	delete timerULSwitcher;
	delete timerDLSwitcher;
}

int OFDMA_PMPBS_MR::init(void)
{
	...
	timerULSwitcher = new timerObj;	
	timerDLSwitcher = new timerObj;

	BASE_OBJTYPE(mem_func);

        mem_func = POINTER_TO_MEMBER(OFDMA_PMPBS_MR, ULzoneSwitch);
        timerULSwitcher->setCallOutObj(this, mem_func);

	mem_func = POINTER_TO_MEMBER(OFDMA_PMPBS_MR, DLzoneSwitch);
        timerDLSwitcher->setCallOutObj(this, mem_func);

	return NslObject::init();
}

int OFDMA_PMPBS_MR::recv(ePacket_ *epkt)
{
	Packet *pkt             = NULL;
	...
	if (log_flag)
	{
		...
		if (!log_info)
		{
			printf("OFDMA_PMPBS_MR::recv(): missing log information.\n");
			exit(1);
		}
	...
	{
		/*Here we delay Access zone at first and process relay later if it existed*/

		if(phyInfo->relay_flag == false)
	...
	//printf("*Time:%llu**BS(%d)::%s,recvState = %d ,UL ticks = %llu***\n", GetCurrentTime(), get_nid(), __func__, recvState, timeInTick);
	timeInTick += GetCurrentTime();
	BASE_OBJTYPE(type);
	type = POINTER_TO_MEMBER(OFDMA_PMPBS_MR, recvHandler);
	setObjEvent(epkt, timeInTick, 0, this, type, epkt->DataInfo_);

	return 1;
}

int OFDMA_PMPBS_MR::send(ePacket_ *epkt)
{
	...
	{
		fprintf(stderr, "\e[1;31mTime:%llu:OFDMA_PMPBS_MR(%d): burstCollection is Empty!\e[0m\n", GetCurrentTime(), get_nid());
		exit(1);
	}
	...
	fec         = QPSK_1_2;
	numBytes    = _channelCoding->getCodedSlotLen(fec) * repeat_times_MR[repe];

	// Get coded length of DL-MAP message
	repe        = REPETATION_CODEWORD_1;
	fec         = QPSK_1_2;
	numBytes    += _channelCoding->getCodedBurstLen(_dlmapLen, repeat_times_MR[repe], fec);
	...
	phyInfo.DLTranspSym = _DLTransparentSymbols;
	phyInfo.flowDir = 'D';
	...
	slots       = _channelCoding->getNumUncodedSlotWithRepe(len * 2, repeat_times_MR[repe], fec);

	for (int i = 0;i < repeat_times_MR[repe];i++)
	{
		n = _channelCoding->encode(tmp_dlfp, output + i * 12, len * 2, slots, repeat_times_MR[repe], fec);
	}
	output += n;
	delete [] tmp_dlfp;
	...
	repe    = REPETATION_CODEWORD_1;
	fec     = QPSK_1_2;
	slots   = _channelCoding->getNumUncodedSlotWithRepe(_dlmapLen, repeat_times_MR[repe], fec);
	n       = _channelCoding->encode(_dlmapBuf, output, _dlmapLen, slots, repeat_times_MR[repe], fec);
	output += n;
	delete _dlmapBuf;
	...
	if (log_flag)
	{
		...
		uint8_t burst_type          = FRAMETYPE_MobileRelayWIMAX_PMP_DABURST;
...
void OFDMA_PMPBS_MR::sendToSubStations(Packet *dlFrame_pkt)
{
	ePacket_ *epkt              = NULL;
	...
	SLIST_FOREACH(wi, &headOfMobileRelayWIMAX_, nextLoc)
	{
		...
		/* We only send the frame to MobileRelayWimax PMPMS & PMPRS module */
		if ((strcmp(nodeType, "MobileRelayWIMAX_PMPMS") != 0) && (strcmp(nodeType, "MobileRelayWIMAX_PMPRS") != 0))
			continue;
...
void OFDMA_PMPBS_MR::recvHandler(ePacket_ *epkt)
{
	...
	//printf("*Time:%llu**BS(%d)::%s,recvState = %d ***\n", GetCurrentTime(), get_nid(), __func__, recvState);
	/* Unsafe state transformation */
	if (recvState == Busy_access || recvState == Busy_transparent)
	{
		/********* LOG START *********/
		if (log_flag)
		{
			...
			if (!log_info)
			{
				fprintf(stderr, "OFDMA_PMPBS_MR::recvHandler(): missing log information.\n");
				exit(1);
			}
			...
			if (phyInfo->relay_flag == true)
				burst_type = FRAMETYPE_MobileRelayWIMAX_PMP_URBURST;
			else
			...
			LOG_MAC_802_16j(mac80216j_log_p1, *start_recv_time, *start_recv_time, get_type(), get_nid(), StartRX,
			...
			DROP_LOG_802_16j(mac80216j_log_p1, mac80216j_log_p2, GetCurrentTime(), DropRX, DROP_COLL);
	...
	if (log_flag)
	{
		...
		/*mac80216j_log_t *mac80216j_log_p1 = (mac80216j_log_t *)malloc(sizeof(mac80216j_log_t));
                ...
                INSERT_TO_HEAP(logep, mac80216j_log_p1->PROTO, mac80216j_log_p1->Time + START, mac80216j_log_p1);
#if LOG_DEBUG
                printf("BS%d %llu ~ %llu %d->%d (StartRX)\n", get_nid(), *start_receive_time, *start_receive_time, log_info->src_nid, get_nid());
#endif*/

		/* log SuccessRX event */
		mac80216j_log_t *mac80216j_log_p2 = (mac80216j_log_t *)malloc(sizeof(mac80216j_log_t));

		LOG_MAC_802_16j(mac80216j_log_p2, GetCurrentTime(), *start_recv_time, get_type(), get_nid(), SuccessRX,
				log_info->src_nid, get_nid(), burst_type, log_info->burst_len, log_info->channel_id);

		INSERT_TO_HEAP(logep, mac80216j_log_p2->PROTO, mac80216j_log_p2->Time + ENDING, mac80216j_log_p2);
#if LOG_DEBUG
		printf("BS%d: %llu ~ %llu %d->%d (SuccessRX)\n",get_nid(), *start_recv_time, GetCurrentTime(), log_info->src_nid, get_nid());
#endif
	}
	/********* LOG END *********/
	...
	{
		if (uiuc == CDMA_Alloc_IE)  /* Special message of CDMA allocation with RNG-REQ pdu*/
		{
...
void OFDMA_PMPBS_MR::buildDLFP(struct DLFP_except_128 *frame_prefix, int dlmapLen)
{
	int numSlots = _channelCoding->getNumUncodedSlotWithRepe(dlmapLen, repeat_times_MR[REPETATION_CODEWORD_1], QPSK_1_2);
...
void OFDMA_PMPBS_MR::buildDLFP(struct DLFP_for_128 *frame_prefix, int dlmapLen)
{
	int numSlots = _channelCoding->getNumUncodedSlotWithRepe(dlmapLen, repeat_times_MR[REPETATION_CODEWORD_1], QPSK_1_2);
...
void OFDMA_PMPBS_MR::saveDLMAP(char *dstbuf, int len)
{
	_dlmapBuf = dstbuf;
	_dlmapLen = len;
}

void OFDMA_PMPBS_MR::setStateBusy()
{
	uint64_t ticks = 0;
	recvState = Busy_access;
	
	MICRO_TO_TICK(ticks, (_DLAccessSymbols) * Ts);
	timerDLSwitcher->start(ticks, 0);		// Idle_trans
		
	MICRO_TO_TICK(ticks, (_DLsymbols + _ULAccessSymbols) * Ts + _TTG * PS);
	timerULSwitcher->start(ticks, 0);
	
}

void OFDMA_PMPBS_MR::setStateIdle()
{
	recvState = Idle_access;
}

/* timer of zone switch */
void OFDMA_PMPBS_MR::ULzoneSwitch()
{
	if (recvState == Idle_access)
		recvState = Idle_relay;
}

void OFDMA_PMPBS_MR::DLzoneSwitch()
{
	if (_DLTransparentSymbols != 0)
	{
        	if (recvState == Busy_access)
                	recvState = Busy_transparent;
	}
	else
		recvState = Idle_access;
}

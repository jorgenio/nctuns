...
mac802_16e::mac802_16e()
{
	...
	mac802_16e::mac802_16e(uint32_t type, uint32_t id, plist* pl, const char* name)
...
int mac802_16e::init()
{
	...
	gen_ranging_code(0x00, code, 256);  // FIXME: Assume UL_PermBase = 0x00

	for (int i = S % 256; i < (S + N) % 256; i++)    // 0 ~ 63
	{
	...
	index = 0;
	for (int i = (S + N) % 256; i < (S + N + M) % 256; i++)  // 64 ~ 127
	{
	...
	index = 0;
	for (int i = (S + N + M) % 256; i < (S + N + M + L) % 256; i++)  // 128 ~ 191
	{
	...
	index = 0;
	for (int i = (S + N + M + L) % 256; i < (S + N + M + L + O) % 256; i++)  // 192 ~ 254
	{
...
int mac802_16e::getCode(int usage)
{
	int index = 0;
...
uint8_t mac802_16e::getCodeIndex(uint8_t *code, uint8_t *usage)
{
...
int mac802_16e::resetTimerT(int tno)
{
...
int mac802_16e::SendNTFYtoPHY(Pkthdr *ntfyCmd)
{
	ePacket_ *epkt  = NULL;
...
int mac802_16e::symbolsToPSs(int nsymbols)
{
	return static_cast<int>(nsymbols * (1 + CPratio()) / PSratio());
}

double mac802_16e::symbolsToMicro(int nsymbols)
{
	return nsymbols * Ts();
}

double mac802_16e::PSsToMicro(int nPSs)
{
	return nPSs * PS();
}

// Convert char* to Packet*
Packet *mac802_16e::asPacket(char *ptr, int pLen)
{
	Packet *pkt = NULL;
	char *buf   = NULL;

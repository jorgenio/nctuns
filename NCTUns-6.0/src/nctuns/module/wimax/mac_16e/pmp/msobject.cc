...
msObject::msObject(uint8_t *pMac, int pBasicCID, int pPriCID, int pSecCID, mac802_16e_PMPBS *pBS)
{
	...
	mem_func = POINTER_TO_MEMBER(msObject, T7);
	timer_mngr()->set_func_t(7u, this, mem_func);

	mem_func = POINTER_TO_MEMBER(msObject, T10);
	timer_mngr()->set_func_t(10u, this, mem_func);

	mem_func = POINTER_TO_MEMBER(msObject, ResourceClear);
...
int msObject::handle(mgmt_msg *recvmm, int cid, int len)
{
 ...
 */
void msObject::procSBCREQ(struct mgmt_msg *recvmm, int cid, int len)
{
 ...
 *    Section 6.3.2.3.10
 */
void msObject::SendDSAREQ(uint16_t cid)
{
	ifmgmt *ifmm        = NULL;
	ifTLV *tmptlv       = NULL;
	uint8_t fhmac[21]   = "";
 ...
 * Figure 70
 */
void msObject::procREGREQ(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm_req    = NULL;
	ifmgmt *ifmm_rsp    = NULL;
	int type            = 0;
...
void msObject::procDSARSP(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm_rsp    = NULL;
...
void msObject::procMOB_MSHOREQ(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm_req            = NULL;
	ifmgmt *ifmm_rsp            = NULL;
	int ifmmLen                 = 0;
	...
	uint8_t BS_RTD              = 0;
	NeighborBSs *tmpList        = new NeighborBSs();
	NbrBS *tmpNbrBS             = NULL;
	...
	for (int i = 0;i < N_BS_Index;i++)
	{
		...
		tmpNbrBS = new NbrBS(-1, servingBS->NeighborBSList->nbrBSs_Index[index]->addr->buf());
	...
	for (int i = 0;i < N_BS_Full;i++)
	{
		...
		tmpNbrBS = new NbrBS(-1, neighborBS_ID);
...
void msObject::procMOB_SCNREQ(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm_req        = NULL;
	ifmgmt *ifmm_rsp        = NULL;
	int ifmmLen             = 0;
...
void msObject::procMOB_SCNREP(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm              = NULL;
	int ifmmLen               = 0;
	uint8_t repMode           = 0;
...
void msObject::procMOB_HOIND(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm    = NULL;
	int ifmmLen     = 0;
	uint8_t rsv     = 0;
	...
	if (Mode == 0x0)
	{
		...
		if (HO_IND_type == 0) // Serving BS release
		{
			...
			targetBS = new NbrBS(-1, targetBSID); // FIXME

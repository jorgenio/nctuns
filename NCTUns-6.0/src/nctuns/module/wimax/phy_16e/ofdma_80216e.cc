...
const double    frameDuration[] = {0, 2.0, 2.5, 4.0, 5.0, 8.0, 10.0, 12.5, 20.0}; // Spec 16e. Table 274.
const int       repeat_times[]  = {1, 2, 4, 6};
extern char     *MobileWIMAXChannelCoding;

	OFDMA_80216e::OFDMA_80216e(uint32_t type, uint32_t id, struct plist *pl, const char *name)
:NslObject(type, id, pl, name)
{
	...
	_symbolsPerFrame = (int) ((frameDuration[frameDuCode] * 1000 - (_TTG + _RTG) * PS) / Ts);

	/* Initial channel model and channel coding */
	if (MobileWIMAXChannelCoding && !strncasecmp(MobileWIMAXChannelCoding, "on", 2))
	{
		_channelCoding  = new OFDMA_ChannelCoding(true);
		_channelModel   = new OFDMA_ChannelModel(BW, Ts, true);
	}
	else
	{
		_channelCoding  = new OFDMA_ChannelCoding(false);
		_channelModel   = new OFDMA_ChannelModel(BW, Ts, false);
	}

	assert(_channelCoding && _channelModel);
}

OFDMA_80216e::~OFDMA_80216e()
{
	delete _channelCoding;
	delete _channelModel;
}

void OFDMA_80216e::scheduleTimer(timerObj *timer, double microSecond)
{
...
void OFDMA_80216e::setChannelID(uint8_t chID)
{
	_ChannelID = chID;
}

uint8_t OFDMA_80216e::getChannelID()
{
	return _ChannelID;
}

void OFDMA_80216e::setDLsymbols(int Nsymbols)
{
	_DLsymbols = Nsymbols;
}

void OFDMA_80216e::setULsymbols(int Nsymbols)
{
	_ULsymbols = Nsymbols;
}

void OFDMA_80216e::dumpByteString(char *prompt, char *byteString, int len)
{
	printf("%s: ", prompt);
	for (int i = 0; i < len; i++)
	{

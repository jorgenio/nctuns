...
bool OFDMA_ChannelCoding::_CC_EncoderTableInitiated   = false;
bool OFDMA_ChannelCoding::_CC_EncoderTableDeleted     = false;
bool OFDMA_ChannelCoding::_CC_DecoderTableInitiated   = false;
bool OFDMA_ChannelCoding::_CC_DecoderTableDeleted     = false;

struct EncoderLookupEntry *OFDMA_ChannelCoding::_CC_1_2_EncoderTable = NULL;
struct EncoderLookupEntry *OFDMA_ChannelCoding::_CC_2_3_EncoderTable = NULL;
struct EncoderLookupEntry *OFDMA_ChannelCoding::_CC_3_4_EncoderTable = NULL;
struct DecoderLookupEntry *OFDMA_ChannelCoding::_CC_1_2_DecoderTable = NULL;
struct DecoderLookupEntry *OFDMA_ChannelCoding::_CC_2_3_DecoderTable = NULL;
struct DecoderLookupEntry *OFDMA_ChannelCoding::_CC_3_4_DecoderTable = NULL;

OFDMA_ChannelCoding::OFDMA_ChannelCoding(bool isEnabled)
{
	...
	_CC_1_2 = new OFDMA_ConvoCode(0171, 0133, 7, 01, 01, 1);
	_CC_2_3 = new OFDMA_ConvoCode(0171, 0133, 7, 02, 03, 2);
	_CC_3_4 = new OFDMA_ConvoCode(0171, 0133, 7, 05, 06, 3);
...
OFDMA_ChannelCoding::~OFDMA_ChannelCoding()
{
	if (_isBypassing)
		return;
...
int OFDMA_ChannelCoding::setEncoderState(char *input, int inputBytes, int j, int n, int k, int m, int codeType)
{
	int index               = 0;
	char state              = 0;
...
// not repeat here
int OFDMA_ChannelCoding::encode(char *input, char *output, int inputBytes, int inputSlots, int repeatTimes, int codeType)
{
	int codedLen    = 0;
	int paddingLen  = 0;
...
int OFDMA_ChannelCoding::encodeBlock(char *input, char *output, int numBlocks, int usedSlot, int codeType)
{
...
int OFDMA_ChannelCoding::decode(char *input, char *output, int inputBytes, int inputSlots, int repeatTimes, int codeType)
{
	int uncodedLen  = 0;
	int uncodedSize = _coding[codeType].uncodedPayload[0]; // per slot
	int codedSize   = _coding[codeType].codedPayload[0];   // per slot
...
int OFDMA_ChannelCoding::decodeBlock(char *input, char *output, char *lastBlock, int lastSlots,
...
void OFDMA_ChannelCoding::initCoding(int uncodedSize, int codedSize, OFDMA_ConvoCode * CC, int Ncpc, int codeType)
{
	/*
...
int OFDMA_ChannelCoding::getNumUncodedSlotWithRepe(int numBytes, int repeatTimes, int codeType)
{
...
int OFDMA_ChannelCoding::getNumCodedSlotWithRepe(int numBytes, int repeatTimes, int codeType)
{
...
// Coded Slot Bytes
int OFDMA_ChannelCoding::getCodedSlotLen(int codeType)
{
	return _coding[codeType].codedPayload[0];
}

// Uncoded Slot Bytes
int OFDMA_ChannelCoding::getUncodedSlotLen(int codeType)
{
	return _coding[codeType].uncodedPayload[0];
}

// Uncoded Bytes ==> Coded Burst Length after apply repetition coding
int OFDMA_ChannelCoding::getCodedBurstLen(int numBytes, int repeatTimes, int codeType)
{
...
int OFDMA_ChannelCoding::getUncodedBurstLen(int numBytes, int codeType)
{
...
void OFDMA_ChannelCoding::randomize(char *input, char *output, int inputBits)
{
	/* Spec. 16e: 8.4.9.1 */
	int seed    = 0x3715;
	int in      = 0;
...
void OFDMA_ChannelCoding::derandomize(char *input, char *output, int inputBits)
{
	randomize(input, output, inputBits);
}

void OFDMA_ChannelCoding::interleave(char *input, char *output, int numSlots, int codeType)
{
...
void OFDMA_ChannelCoding::deinterleave(char *input, char *output, int numSlots, int codeType)
{
	int Ncbps   = _coding[codeType].Ncbps[numSlots - 1];
	int Ncpc    = _coding[codeType].Ncpc;
	int s       = Ncpc / 2;
...
void OFDMA_ChannelCoding::dumpByteString(char *prompt, char *byteString, int len)
{
	printf("%s:\t", prompt);
	for (int i = 0; i < len; i++) 
	{
...
void OFDMA_ChannelCoding::dumpByteByBits(char b)
{
	for (int i = 0;i < 8;i++)
	{
		printf("%X", ((b << i) & 0x80) >> 7);

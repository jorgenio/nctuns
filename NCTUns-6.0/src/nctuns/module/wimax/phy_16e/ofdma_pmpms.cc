...
#include "ofdma_pmpms.h"

MODULE_GENERATOR(OFDMA_PMPMS);

extern SLIST_HEAD(wimax_head, con_list) headOfMobileWIMAX_;
extern typeTable *typeTable_;
extern char *MobileWIMAXChannelCoding;

	OFDMA_PMPMS::OFDMA_PMPMS(uint32_t type, uint32_t id, struct plist *pl, const char *name)
:OFDMA_80216e(type, id, pl, name)
{
	...
	assert(_recvTimer);
	func = POINTER_TO_MEMBER(OFDMA_PMPMS, recvHandler);
	_recvTimer->setCallOutObj(this, func);
}

OFDMA_PMPMS::~OFDMA_PMPMS()
{
	delete _recvTimer;
}

int OFDMA_PMPMS::init(void)
{
...
int OFDMA_PMPMS::recv(ePacket_ *epkt)
{
	double          recvPower   = 0;
	double          *dist       = NULL;
	Packet          *pkt        = NULL;
	...
	if (recvState != Idle)
	{
		fprintf(stderr, "\e[1;31mTime:%llu:OFDMA_PMPMS(%d)::recv() PHY is busy now! Receive State (%d) Error\e[0m\n", GetCurrentTime(), get_nid(), recvState);
		exit(1);
	}
...
int OFDMA_PMPMS::send(ePacket_ *epkt)
{
	...
	{
		fprintf(stderr, "\e[1;31mMS(%d)::OFDMA_PMPMS::%s(): burstCollection->size() != 1\e[0m\n", get_nid(), __func__);
		exit(1);
	}
	...
	codedLen    = _channelCoding->getCodedBurstLen(uncodedLen, repeat_times[repe], fec);
	...
	slots       = _channelCoding->getNumUncodedSlotWithRepe(uncodedLen, repeat_times[repe], fec);
	_channelCoding->encode(input, output, uncodedLen, slots, repeat_times[repe], fec);
...
void OFDMA_PMPMS::sendToBS(Packet *ULburst, struct LogInfo *log_info)
{
	ePacket_ *epkt              = NULL;
	...
	SLIST_FOREACH(wi, &headOfMobileWIMAX_, nextLoc)
	{
		...
		if (strcmp(nodeType, "MobileWIMAX_PMPBS") != 0)
			continue;
		...
		if (log_flag)
		{
			...
			double trans_time           = _ULsymbols * Ts;
...
void OFDMA_PMPMS::recvHandler()
{
	...
	if (src_log_info == NULL)  /* Assertion for struct LogInfo structure */
	{
		if (log_flag == 1)
		{
			fprintf(stderr, "OFDMA_PMPMS::recvHandler(): source log information is lost.\n");
			exit(1);
		}
	}
	else
	...
	{
		fprintf(stderr, "OFDMA_PMPMS::recvHandler(): Frame receive time is lost.\n");
		exit(1);
	}
	...
	else if (recvState == ProcDLFP)    /* Process DLFP and DL-MAP */
	{
		...
		fec     = QPSK_1_2;
		len     = _channelCoding->getCodedSlotLen(fec) * repeat_times[repe];
		...
		/********** LOG **************/
		burst_type = FRAMETYPE_MobileWIMAX_PMP_DLFP;
		...
		BER = _channelModel->computeBER(fec, saveSNR);
		_channelModel->makeBitError(_burstPtr, len, BER);
		slots = _channelCoding->getNumCodedSlotWithRepe(len, repeat_times[repe], fec);
		_channelCoding->decode(_burstPtr, output, len, slots, repeat_times[repe], fec);
		_burstPtr += len;
	...
	else if (recvState == ProcIE)  /* Process the DL bursts depend on the received DL information element */
	{
		dlmapinfo *ieInfo = NULL;

		while (_DLMapInfo.size() != 0)
		{
			/* Get an IE to compute the burst length */
			ieInfo      = _DLMapInfo.front();
			...
			/********** LOG START **************/
			burst_type = FRAMETYPE_MobileWIMAX_PMP_DBURST;
			log_info.burst_len = burstLen;
			/********** LOG END **************/
			...
			delete *(_DLMapInfo.begin());
			_DLMapInfo.erase(_DLMapInfo.begin());
		}
	...
	{
		fprintf(stderr, "\e[1;31mTime:%llu:OFDMA_PMPMS(%d):Unknown state\e[0m\n", GetCurrentTime(), get_nid());
		exit(1);
	}
	...
	if (log_flag)
	{
		...
		if (burst_type == FRAMETYPE_MobileWIMAX_PMP_DLFP)
			trans_time = DL_PUSC * Ts;
		else
			trans_time= (_DLsymbols - DL_PUSC - PREAMBLE) * Ts;
		...
		LOG_MAC_802_16e(mac80216e_log_p1, start_receive_time, start_receive_time, get_type(), get_nid(), StartRX,
		...
		LOG_MAC_802_16e(mac80216e_log_p2, start_receive_time + trans_time_in_tick, start_receive_time, get_type(), get_nid(), SuccessRX,
				src_nid, log_info.dst_nid, burst_type, log_info.burst_len, log_info.channel_id);
...
void OFDMA_PMPMS::decodeDLFP(char *input, struct DLFP_except_128 *dlfp_p)
{
	char *tmp       = NULL;
...
int OFDMA_PMPMS::procRNGCode(Pkthdr *pkthdr)
{
	Packet *ulpkt           = NULL;
	char *output            = NULL;
...
int OFDMA_PMPMS::procCmdFromMAC(Pkthdr *pkthdr)
{
	...
	scheduleTimer(_recvTimer, (_DLsymbols - DL_PUSC - PREAMBLE) * Ts);

	return 1;
}

int OFDMA_PMPMS::skipBufferedPkt()
{
...
void OFDMA_PMPMS::dump_buf(int st, int en)
{

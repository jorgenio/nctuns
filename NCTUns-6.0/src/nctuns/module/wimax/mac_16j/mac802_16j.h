...
#ifndef __NCTUNS_MAC_80216J_H__
#define __NCTUNS_MAC_80216J_H__
...
namespace mobileRelayMac80216j {
	class mac802_16j:public NslObject {
		private:
			Mac_address *_addr;
			double      *_Ts;
			double      *_PS;
		...
		protected:
			...
			uint8_t _initRang_code_db[43][18];      // each 144 bits
                        uint8_t _periodRang_code_db[43][18];    // each 144 bits
                        uint8_t _bwRequest_code_db[43][18];     // each 144 bits
                        uint8_t _handoverRang_code_db[43][18];  // each 144 bits
			uint8_t _rs_initRang_code_db[43][18];
			uint8_t _rs_dedicated_code_db[40][18];
			...
			inline int    DLsubchannels()   const { return *_DLsubchannels; }
			inline int    ULsubchannels()   const { return *_ULsubchannels; }
			inline int    TTG()             const { return *_TTG; }
			inline int    RTG()             const { return *_RTG; }
			inline int    RSTTG()             const { return *_RSTTG; }
			inline int    RSRTG()             const { return *_RSRTG; }
		...
		public:
			explicit mac802_16j();
			explicit mac802_16j(uint32_t, uint32_t, plist *, const char *);
			virtual ~mac802_16j();

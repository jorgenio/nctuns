...
NbrBS_MR::NbrBS_MR(int _nid, uint8_t *_addr)
{
	nid     = _nid;
...
NbrBS_MR *NeighborBSs_MR::getNbrbyBSID(uint8_t *_addr)
{
...
NbrBS_MR *NeighborBSs_MR::getNbrbyChID(uint8_t chID)
{
	vector<NbrBS_MR *>::iterator iter;
...
int NeighborBSs_MR::getNextScanChID()
{
	vector<NbrBS_MR *>::iterator iter;
	uint32_t Index_i    = 0;
	uint32_t Full_i     = 0;
	...
	{
		return nbrBSs_Index[0]->PreambleIndex;	// first scan CH ID
	}
}

bool NeighborBSs_MR::checkHOterms()
{
	vector<NbrBS_MR *>::iterator iter;
...
NbrBS_MR *NeighborBSs_MR::getTargetBS()  // Now we choose the best CINR to be the target BS
{
	vector<NbrBS_MR *>::iterator iter;
	NbrBS_MR *trgetBS  = NULL;
	int max_CINR    = 0;

...
//using namespace MRSmsObject_mr;

msObject_mr::msObject_mr(uint8_t *pMac, int pBasicCID, int pPriCID, int pSecCID, mac802_16j_PMPBS *pBS, mac802_16j_PMPMS *pMS)
{
	...
	mem_func = POINTER_TO_MEMBER(msObject_mr, T7);
	timer_mngr()->set_func_t(7u, this, mem_func);

	mem_func = POINTER_TO_MEMBER(msObject_mr, T10);
	timer_mngr()->set_func_t(10u, this, mem_func);

	mem_func = POINTER_TO_MEMBER(msObject_mr, ResourceClear);
	timerResourceRetain->setCallOutObj(this, mem_func);
}

msObject_mr::~msObject_mr()
{
...
int msObject_mr::handle(mgmt_msg *recvmm, int cid, int len)
{
	...
	if (cid == BasicCID)   // Basic CID
	{
		...
		if (recvmm->type == MG_REPRSP) // REPRSP
                {
                        procREPRSP(recvmm, cid, len);
                }
	}
	else if (cid == PriCID)   // Primary CID
	{
		...
		if (recvmm->type == MG_REGREQ)  // REG-REQ
		{
			...
			if (DSApending == broadcastCID)
			{
				...
				/* Encode RS_MemberListUpdate if needed */
				if (accessStation != servingBS)
				{
					pRelayRS = (rsObject*) accessStation;
					list_add_flag = true;
					cidlist.push_back(conn->getCID());
					servingBS->SendRS_MemberListUpdate(cidlist, list_add_flag, pRelayRS->getBasicCID());
				}
			}
		}
 ...
 */
void msObject_mr::ResourceClear()
{
	//printf("%d: msObject_mr::%s()\n", get_nid(), __func__);
 ...
 */
void msObject_mr::procSBCREQ(struct mgmt_msg *recvmm, int cid, int len)
{
	...
	//printf("\nTime:%llu:msObject_mr::SendSBCRSP()\n", GetCurrentTime());
	// SBC-RSP
...
void msObject_mr::procREPRSP(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt	*ifmm = NULL;
	int	type = 0;
	uint8_t frdiuc = 0;
	...
	if (servingBS != accessStation)
	{	
		pRS = (rsObject*)accessStation;

		if (frsbcid == pRS->getBasicCID())
		{
			rdiuc = frdiuc;
			myMS->SetRDIUC(frdiuc);
 ...
 *    Section 6.3.2.3.10
 */
void msObject_mr::SendDSAREQ(uint16_t cid)
{
	ifmgmt *ifmm        = NULL;
	ifTLV *tmptlv       = NULL;
	uint8_t fhmac[21]   = "";
 ...
 * Figure 70
 */
void msObject_mr::procREGREQ(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm_req    = NULL;
	ifmgmt *ifmm_rsp    = NULL;
	int type            = 0;
	...
	ifmm_rsp = new ifmgmt(MG_REGRSP, 1);    // Sec 11.8
        ifmm_rsp->appendField(1,0);
...
void msObject_mr::procDSARSP(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm_rsp    = NULL;
...
void msObject_mr::procMOB_MSHOREQ(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm_req            = NULL;
	ifmgmt *ifmm_rsp            = NULL;
	int ifmmLen                 = 0;
	...
	uint8_t BS_RTD              = 0;
	NeighborBSs_MR *tmpList        = new NeighborBSs_MR();	// List of the BSs suit to be target BS
	NbrBS_MR *tmpNbrBS             = NULL;
	...
	for (int i = 0;i < N_BS_Index;i++)
	{
		...
		tmpNbrBS = new NbrBS_MR(-1, servingBS->NeighborBSList->nbrBSs_Index[index]->addr->buf());
		tmpNbrBS->Index = index;

		ifmm_req->extractBitField( 8, &tmpNbrBS->PreambleIndex);
		//printf("=====[%d] Channel ID: %d\n", i, tmpNbrBS->PreambleIndex);

		tmpBits += 16;

		if ((tmpList->ReportMetric & 0x01) == 1)
		{
			ifmm_req->extractBitField( 8, &tmpNbrBS->CINR);
			//printf("=====[%d] CINR: %d\n", i, tmpNbrBS->CINR);
			tmpBits += 8;
		}
		...
		{
			ifmm_req->extractBitField( 8, &tmpNbrBS->RSSI);
			tmpBits += 8;
		}

		if (((tmpList->ReportMetric & 0x04) >> 2) == 1)
		{
			ifmm_req->extractBitField( 8, &tmpNbrBS->RelativeDelay);
			tmpBits += 8;
		}
		...
		tmpList->nbrBSs_Index.push_back(tmpNbrBS);
	}
	...
	for (int i = 0;i < N_BS_Full;i++)
	{
		...
		tmpNbrBS = new NbrBS_MR(-1, neighborBS_ID);
		...
		tmpList->nbrBSs_Full.push_back(tmpNbrBS);
	}
	...
	if (Mode == 0x00)
	{
		...
		if (CfgChangeCnt == servingBS->NeighborBSList->NBRADV_CfgCount)
		{
...
void msObject_mr::procMOB_SCNREQ(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm_req        = NULL;
	ifmgmt *ifmm_rsp        = NULL;
	int ifmmLen             = 0;
	...
	//printf("\nTime:%llu:MS(%d): msObject_mr::%s()\n", GetCurrentTime(), get_nid(), __func__);

	/*
	 * Extract MOB_SCN-REQ Message
	 */
	ifmm_req = new ifmgmt((uint8_t *) recvmm, len, len - 1);
	ifmm_req->extractBitField( 8, &scanDuration);	// maximum = 5
	ifmm_req->extractBitField( 8, &interleave);	// 10  frames
	ifmm_req->extractBitField( 8, &scanIterate);	// 1
	...
	if ((CfgChangeCnt !=servingBS->NeighborBSList->NBRADV_CfgCount || N_RecBS_Index == 0) && N_RecBS_Full == 0)
	{
		scanDuration = 0;
	}
	...
	if (scanDuration != 0)
	{
		...
		if (CfgChangeCnt == servingBS->NeighborBSList->NBRADV_CfgCount)
		{
			...
			if (N_RecBS_Index != 0)
			{
				ifmm_rsp->appendBitField( 8, servingBS->NeighborBSList->NBRADV_CfgCount);
			}
...
void msObject_mr::procMOB_SCNREP(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm              = NULL;
	int ifmmLen               = 0;
	uint8_t repMode           = 0;
...
void msObject_mr::procMOB_HOIND(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm    = NULL;
	int ifmmLen     = 0;
	uint8_t rsv     = 0;
	...
	if (Mode == 0x0)
	{
		...
		if (HO_IND_type == 0) // Serving BS release
		{
			...
			targetBS = new NbrBS_MR(-1, targetBSID); // FIXME

			// Start the Resource retain timer
			MILLI_TO_TICK(ticks, ResourceRetainTime * 100);	// 2s 
			timerResourceRetain->start(ticks, 0);
			ResourceStatus = PerformHO;
			/* Erase relay RS connection iff MS relayed */
			if(accessStation != servingBS)
                        {
                                servingBS->SendMS_InfoDel(BasicCID, relayRSBcid);
                        }

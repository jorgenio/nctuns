...
rsObject::rsObject(uint8_t *pMac, int pBasicCID, int pPriCID, int pSecCID, mac802_16j_PMPBS *pBS, mac802_16j_PMPRS* pRS)
{
	...
	mem_func = POINTER_TO_MEMBER(rsObject, T7);
	timer_mngr()->set_func_t(7u, this, mem_func);

	mem_func = POINTER_TO_MEMBER(rsObject, T10);
	timer_mngr()->set_func_t(10u, this, mem_func);

	mem_func = POINTER_TO_MEMBER(rsObject, T68);
        timer_mngr()->set_func_t(68u, this, mem_func);

	mem_func = POINTER_TO_MEMBER(rsObject, ResourceClear);
...
int rsObject::handle(mgmt_msg *recvmm, int cid, int len, int cur_frame_no)
{
	...
	if (cid == BasicCID)   // Basic CID
	{
		...
		if (recvmm->type == MG_MR_GenericACK) // MOB_SCN-REQ
                {
 ...
 */
void rsObject::procSBCREQ(struct mgmt_msg *recvmm, int cid, int len)
{
 ...
 *    Section 6.3.2.3.10
 */
void rsObject::SendDSAREQ(uint16_t cid)
{
	ifmgmt *ifmm        = NULL;
	ifTLV *tmptlv       = NULL;
	uint8_t fhmac[21]   = "";
...
void rsObject::SendRCD()
{
	;
}

/*
 * 16j Spec.D7 6.3.9.18.1 & 6.3.2.3.64
 */
void rsObject::SendRS_ConfigCMD(int frame_no)
{
	...
	ifmm->appendField(2, 1234);    // Transaction ID
	ifmm->appendField(1, frame_no & 0x0F);	//8 bits LSB of frame number action
 ...
 * Figure 70
 */
void rsObject::procREGREQ(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm_req    = NULL;
	ifmgmt *ifmm_rsp    = NULL;
	int type            = 0;
	...
	ifmm_rsp->appendField(1,0);		//append response field with OK(0)	
...
void rsObject::procDSARSP(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm_rsp    = NULL;
...
void rsObject::procMOB_MSHOREQ(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm_req            = NULL;
	ifmgmt *ifmm_rsp            = NULL;
	int ifmmLen                 = 0;
	...
	uint8_t BS_RTD              = 0;
	NeighborBSs_MR *tmpList        = new NeighborBSs_MR();
...
void rsObject::procMOB_SCNREQ(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm_req        = NULL;
	ifmgmt *ifmm_rsp        = NULL;
	int ifmmLen             = 0;
...
void rsObject::procMOB_SCNREP(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm              = NULL;
	int ifmmLen               = 0;
	uint8_t repMode           = 0;
...
void rsObject::procMOB_HOIND(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm    = NULL;
	int ifmmLen     = 0;
	uint8_t rsv     = 0;
	...
	if (Mode == 0x0)
	{
		...
		if (HO_IND_type == 0) // Serving BS release
		{
			...
			MILLI_TO_TICK(ticks, ResourceRetainTime * 100);		//unit in 100 ms
			timerResourceRetain->start(ticks, 0);
			ResourceStatus = RS_PerformHO;
...
void rsObject::procMR_GenericACK(struct mgmt_msg *recvmm, int cid, int len)
{
	ifmgmt *ifmm     = NULL;
        uint16_t ftransid   = 0;

	//printf("\n\e[1;33mTime:%llu MR-BS %s()\e[0m\n", GetCurrentTime(), __func__);

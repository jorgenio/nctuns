...
#ifndef __NCTUNS_80216J_STRUCTURE_H__
#define __NCTUNS_80216J_STRUCTURE_H__
...
struct DCDBurstProfile {
	char used;
	char fec;
	/* 16j added , purpose for path selection*/
	double weight;
};
...
struct PHYInfo {
	...
	int	 ULallocStartTime;  // In ticks
	ULMAP_IE_u ulmap_ie;        // UL-MAP IE
	
	/* 16j new added */
	bool RS_RNGREQ_flag;	    // Comparation between RS & MS RNG-REQ
	uint32_t nid;		    // purpose to msobject and rsobject to find out corresponding MS/RS module
	uint32_t pid;
	bool relay_flag; 	    // indicate if flow be transmitted via RS or not 
	char flowDir;               // Discribe flow direction to relay RS (U/D)
	int ULAccessSym;	    // Record UL access symbols
	int ULRelaySym;		    // Record UL relay symbols
	int DLAccessSym;	    // Record DL access symbols
	int DLTranspSym;	    // Record DL transparent symbols
	uint16_t MSBcid;	    // Record MS basic cid
	uint16_t RSBcid;            // Record RS basic cid
	uint8_t d_diuc;		    // Record measuring diuc from BS to MS/RS

...
void Connection::BuildGenericHeader(char *buf, int len)
{
	...
	hg->cid_msb = cid / 256;
	hg->cid_lsb = cid % 256;
	hg->hcs     = hcs_16j(reinterpret_cast <char*>(hg),sizeof(struct hdr_generic) - 1);
	...
	if (crcIndicator != 0) {
		...
		crc = crc32_16j(0, (char *) buf, len - 4);
...
ManagementConnection::~ManagementConnection()
{
	while(!pduQ.empty())
	{
		delete (ManagementMessage *)*(pduQ.begin());
...
void BroadcastConnection::Insert(const ManagementMessage *pdu)
{
	...
	static int index[NumBroadcast] = { MG_DLMAP, MG_ULMAP, MG_DCD, MG_UCD, MG_CLKCMP, MG_MOB_TRFIND, MG_MOB_NBRADV, MG_MOB_PAGADV};
...
int DataConnection::EncapsulateAll(char* dstbuf, size_t maxlen)
{
	...
	if (_qPkt)	//append latest frangment packet tail to new pdu
	{
		hlen = GetHeaderLength() + 1;	 //fragmentation subheader
	...
	while (!_qPkt && !pduQ.empty())
	{
		...
		sendlen += pdulen;
		ptr += pdulen;
		/* if left room only enough or less to encoding header , we give up continuing encoding */
...
/* for package fragmentation packet purpose*/
...
{
	char *ret = NULL;
	/* if pack fragmentation packet complete , return buffer packet */
	if (_state == Complete)
	{
		ret = _buffer;
		len = _len;

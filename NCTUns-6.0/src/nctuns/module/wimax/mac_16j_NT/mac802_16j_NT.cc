...
mac802_16j_NT::mac802_16j_NT()
{
	...
	mac802_16j_NT::mac802_16j_NT(uint32_t type, uint32_t id, plist* pl, const char* name)
...
int mac802_16j_NT::init()
{
	...
	_RTTI               = GET_REG_VAR(get_port(), "RTTI", int *);
	_RRTI               = GET_REG_VAR(get_port(), "RRTI", int *);

	assert(_Ts && _PS && _Pt && _CPratio && _PSratio && _symbolsPerFrame
			&& _DLsubchannels && _ULsubchannels && _TTG && _RTG && _RTTI && _RRTI);

	// Generate Ranging codes (Spec. 16e. 8.4.7.3)
	gen_ranging_code_NT(0x00, code, 256);  // FIXME: Assume UL_PermBase = 0x00
...
int mac802_16j_NT::getCode(int usage)
{
	int index = 0;
...
uint8_t mac802_16j_NT::getCodeIndex(uint8_t *code, uint8_t *usage)
{
...
int mac802_16j_NT::resetTimerT(int tno)
{
...
int mac802_16j_NT::SendNTFYtoPHY(Pkthdr *ntfyCmd)
{
	ePacket_ *epkt  = NULL;
...
int mac802_16j_NT::symbolsToPSs(int nsymbols)
{
	return static_cast<int>(nsymbols * (1 + CPratio()) / PSratio());
}

double mac802_16j_NT::symbolsToMicro(int nsymbols)
{
	return nsymbols * Ts();
}

double mac802_16j_NT::PSsToMicro(int nPSs)
{
	return nPSs * PS();
}

// Convert char* to Packet*
Packet *mac802_16j_NT::asPacket(char *ptr, int pLen)
{
	Packet *pkt = NULL;
	char *buf   = NULL;

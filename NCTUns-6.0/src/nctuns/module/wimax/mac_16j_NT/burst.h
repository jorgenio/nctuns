...
#ifndef __NCTUNS_80216J_NT_BURST_H__
#define __NCTUNS_80216J_NT_BURST_H__
...
namespace MR_Burst_NT {
	class AllocInfo {
		public:
			Connection *conn;
			BR_Connection *br_conn;
			int len;
			vector<int> pduInfo;
			uint32_t position;
			...
			AllocInfo(BR_Connection *c, int l, vector<int> p)
			{
				br_conn = c;
				len = l;
				pduInfo = p;
	...
	class WiMaxBurst {
		...
		public:
            ...
            void Dump_WiMaxBurst()
            {
                printf("WiMaxBurst : _fec = %d\n",_fec);
                printf("WiMaxBurst : length = %d (bytes)\n",length);
            }

            void Clear_WiMaxBurst()
            {
             //   int size = sizeof(AllocInfo)*ConnectionCollection.size();
             //   printf("*** Time:%llu Clear ConnectionCollection (AllocInfo) size=%d ***\n",GetCurrentTime(),size);
				AllocInfo *a = NULL;
	...
	class downBurst:public WiMaxBurst {
		public:
			...
			~downBurst()
			{
				if (ie != NULL)
				{
					if ((ie->_diuc != Extended_DIUC_2 && ie->_diuc != Extended_DIUC && ie->_diuc != 13) && (nCid != 0))
					{
			...
			{
                // 44 + 16*nCid bits
				if (ie->_diuc != Extended_DIUC_2 && ie->_diuc != Extended_DIUC && ie->_diuc!=13)
				{
					ie->mallocIE(nCid);
	...
	class upBurst:public WiMaxBurst {
		public:
			...
			inline void encapsulateAllField()
			{
				if (ie->_uiuc == CDMA_BWreq_Ranging)
				{
					ie->mallocIE(0);
				...
				else if (ie->_uiuc == CDMA_Alloc_IE)
				{
					ie->mallocIE(0);
...
#endif                /* __NCTUNS_80216J_NT_BURST_H__ */

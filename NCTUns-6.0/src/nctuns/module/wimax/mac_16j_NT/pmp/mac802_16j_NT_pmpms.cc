...
MODULE_GENERATOR(mac802_16j_NT_PMPMS);

	mac802_16j_NT_PMPMS::mac802_16j_NT_PMPMS(uint32_t type, uint32_t id, plist* pl, const char* name)
: mac802_16MSbase(type, id, pl, name)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::mac802_16j_NT_PMPMS State: %x",get_nid(),State);
	...
	pScheduler      = new MS_16j_NT_Scheduler(this);
	...
	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, T1);
	timer_mngr()->set_func_t(1u, this, mem_func);

	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, T2);
	timer_mngr()->set_func_t(2u, this, mem_func);

	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, T3);
	timer_mngr()->set_func_t(3u, this, mem_func);

	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, T6);
	timer_mngr()->set_func_t(6u, this, mem_func);

	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, T8);
	timer_mngr()->set_func_t(8u, this, mem_func);

	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, T12);
	timer_mngr()->set_func_t(12u, this, mem_func);

	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, T18);
	timer_mngr()->set_func_t(18u, this, mem_func);

	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, T42);
	...
	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, LostDLMAP);
	timerLostDL->setCallOutObj(this, mem_func);

	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, LostULMAP);
	timerLostUL->setCallOutObj(this, mem_func);

	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, SendRNGCODE);
	timerSendRngCode->setCallOutObj(this, mem_func);

	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, SendRNGREQ);
	timerSendRngReq->setCallOutObj(this, mem_func);

	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, PacketScheduling);
	timerSendUL->setCallOutObj(this, mem_func);

	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, HOmsgRetransmit);
	timerHORetransmit->setCallOutObj(this, mem_func);

	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, StartPeriodRanging);
	timerPeriodRanging->setCallOutObj(this, mem_func);

	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, SendMOB_SCNREQ);
	timerSendScanReq->setCallOutObj(this, mem_func);

	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, StartScanNbrMRBS_NTs);
	timerStartScan->setCallOutObj(this, mem_func);

	mem_func = POINTER_TO_MEMBER(mac802_16j_NT_PMPMS, StartHandover);
...
mac802_16j_NT_PMPMS::~mac802_16j_NT_PMPMS()
{
...
int mac802_16j_NT_PMPMS::init()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::init State: %x",get_nid(),State);
	...
	NeighborMRBSList->ServingBSchID = ((OFDMA_PMPMS_NT *)sendtarget_->bindModule())->getChannelID();
	NeighborMRBSList->NBRADV_CfgCount = 0;

	return mac802_16j_NT::init();
}

int mac802_16j_NT_PMPMS::recv(ePacket_ *epkt)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::recv State: %x",get_nid(),State);
	...
	if (ScanFlag == true)
	{
		...
		((OFDMA_PMPMS_NT *)sendtarget_->bindModule())->skipBufferedPkt();
	...
	for (ptr = adaptedBurst;ptr + sizeof(struct hdr_generic) < adaptedBurst + burstLen;ptr += plen)
	{
		...
		if (cid == broadcastCID)
		{
            //printf("mac802_16j_NT_PMPMS::recv Time:%llu cid=%d\n",GetCurrentTime(),cid);
		...
		else // Secondary CID or Transport CID
		{
            ...
            //if(!(LastSignalInfo.relay_flag))    // XOR = 0 (both true or false)
            //{
...
int mac802_16j_NT_PMPMS::send(ePacket_ *epkt)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::send State: %x",get_nid(),State);
    ...
    //if(get_nid() == 6)
    //printf("Time:%llu mac802_16j_NT_PMPMS[%d]::%s()\n",GetCurrentTime(),get_nid(),__func__);
...
Connection *mac802_16j_NT_PMPMS::getConnection(uint16_t cid)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::getConnection State: %x",get_nid(),State);
...
void mac802_16j_NT_PMPMS::PushPDUintoConnection(uint16_t cid, ifmgmt *pdu)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::PushPDUintoConnection State: %x",get_nid(),State);
 ...
 */
uint64_t mac802_16j_NT_PMPMS::computeSendTime(int numSlots, int duration, int fastime)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::computeSendTime State: %x",get_nid(),State);
 ...
 */
uint64_t mac802_16j_NT_PMPMS::computeSendTime(const struct OFDMA_ULMAP_IE_12 &ie_12, int txopOffset, int fastime)
{
 ...
 */
void mac802_16j_NT_PMPMS::PacketScheduling()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::PacketScheduling State: %x",get_nid(),State);
 ...
 */
void mac802_16j_NT_PMPMS::T1()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::T1 State: %x",get_nid(),State);

	printf("\nTime:%llu:MS(%d): mac802_16j_NT_PMPMS::%s()\n", GetCurrentTime(), get_nid(), __func__);
}

/*
 * Wait for Broadcast Ranging timeout
 */
void mac802_16j_NT_PMPMS::T2()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::T2 State: %x",get_nid(),State);
 ...
 */
void mac802_16j_NT_PMPMS::T3()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::T3 State: %x",get_nid(),State);
 ...
 */
void mac802_16j_NT_PMPMS::T6()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::T6 State: %x",get_nid(),State);
 ...
 */
void mac802_16j_NT_PMPMS::T8()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::T8 State: %x",get_nid(),State);
 ...
 */
void mac802_16j_NT_PMPMS::T12()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::T12 State: %x",get_nid(),State);
 ...
 */
void mac802_16j_NT_PMPMS::T18()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::T18 State: %x",get_nid(),State);
 ...
 */
void mac802_16j_NT_PMPMS::T42()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::T42 State: %x",get_nid(),State);

	printf("\nTime:%llu:MS(%d): mac802_16j_NT_PMPMS::%s() ... HO holding down timeout... \n", GetCurrentTime(), get_nid(), __func__);
 ...
 */
void mac802_16j_NT_PMPMS::LostDLMAP()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::LostDLMAP State: %x",get_nid(),State);
 ...
 */
void mac802_16j_NT_PMPMS::LostULMAP()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::LostULMAP State: %x",get_nid(),State);
 ...
 */
void mac802_16j_NT_PMPMS::HOmsgRetransmit()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::HOmsgRetransmit State: %x",get_nid(),State);

	uint64_t tick = 0;

	printf("\n\e[1;31mTime:%llu:MS(%d): mac802_16j_NT_PMPMS::%s()\e[0m\n", GetCurrentTime(), get_nid(), __func__);
 ...
 */
void mac802_16j_NT_PMPMS::StartPeriodRanging()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::StartPeriodRanging State: %x",get_nid(),State);
 ...
 */
void mac802_16j_NT_PMPMS::StartScanNbrMRBS_NTs()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::StartScanNbrMRBS_NTs State: %x",get_nid(),State);
	...
	if (NeighborMRBSList->ScanTimes != NeighborMRBSList->ScanDuration) // continue scanning
	{
		...
		// Continue scanning
		if (((OFDMA_PMPMS_NT *)sendtarget_->bindModule())->getChannelID() == chID)
		{
		...
		((OFDMA_PMPMS_NT *)sendtarget_->bindModule())->setChannelID(chID);
 ...
 */
void mac802_16j_NT_PMPMS::StartHandover()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::StartHandover State: %x",get_nid(),State);
	...
	// Change Channel ID
	((OFDMA_PMPMS_NT *)sendtarget_->bindModule())->setChannelID(NeighborMRBSList->getTargetBS()->PreambleIndex);
 ...
 */
void mac802_16j_NT_PMPMS::WaitForRangingOpportunity()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::WaitForRangingOpportunity State: %x",get_nid(),State);
 ...
 *    Processing DCD
 */
int mac802_16j_NT_PMPMS::procDCD(struct mgmt_msg *recvmm, int cid, int len)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::procDCD State: %x",get_nid(),State);
	...
	//printf("\nTime:%llu:mac802_16j_NT_PMPMS::%s()\n", GetCurrentTime(), __func__);
 ...
 *    Processing UCD
 */
int mac802_16j_NT_PMPMS::procUCD(struct mgmt_msg *recvmm, int cid, int len)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::procUCD State: %x",get_nid(),State);
 ...
 * Processing DL-MAP
 */
int mac802_16j_NT_PMPMS::procDLMAP(struct mgmt_msg *recvmm, int cid, int len)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::procDLMAP State: %x",get_nid(),State);
	...
	//printf("\nTime:%llu:mac802_16j_NT_PMPMS(%d)::%s()\n", GetCurrentTime(), get_nid(), __func__);
	ifmm = new ifmgmt((uint8_t *) recvmm, len, len - 1); // len - 1, because the first field is type
	ifmm->extractBitField( 8, (uint8_t *)&frameDuration_NT);
	...
	((OFDMA_PMPMS_NT *)sendtarget_->bindModule())->setDLsymbols(numSymbols);
		...
		while(1)
		{
			...
			switch(diuc) {
				case Gap_PAPR_reduction: // diuc=13 ==> relay zone indicator
                    ...
                    //printf("MS::%s , relay_zone_ind = %d\n",__func__,ie_13->relay_zone_ind);
                    relay_ind = true;
                    leftBits -= 36;
                    delete ie_13;
                    break;
			...
			if(ie_other != NULL)
			{
                ...
                else
                {
                    if(!ntfyCmd_access)
                    {
                        ntfyCmd_access = new ntfyDLMAP;
                        ntfyCmd_access->relay_ind = relay_ind;
                    }
                    ntfyCmd_access->info.push_back(info);
                }
 ...
 * Processing UL-MAP
 */
int mac802_16j_NT_PMPMS::procULMAP(struct mgmt_msg *recvmm, int cid, int len)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::procULMAP State: %x",get_nid(),State);
    ...
    //printf("Time:%llu MS[%d]::%s() fastime=%d(PSs)\n",GetCurrentTime(),get_nid(),__func__,fastime);
    ((OFDMA_PMPMS_NT *)sendtarget_->bindModule())->setULsymbols(ULsymbols);
	...
	while(1)
	{
		...
		switch(ie_uiuc) {
				...
				if (flag_IHie == false)
				{
					...
					if (RngLastStatus == Rerange)  // just synchronize to UL
					{
						...
						else  // Send Initial Ranging Code at Initial Ranging Region
						{
                            ...
                            uint64_t tick1;
                            MICRO_TO_TICK(tick1,PSsToMicro(ULallocStartTime));
                            tick1 += LastSignalInfo.frameStartTime;
                            //printf("\n[%d]Time:%llu MS(%d)::%s | fNumber=%d fSTime=%llu ULaSTime = %llu\n",__LINE__,GetCurrentTime(),get_nid(),__func__,frameNumber & 0xFF,LastSignalInfo.frameStartTime,tick1);
							//printf("\e[1;35m(%d)Time:%llu MS(%d) timerSendRngCode->start (Initial) RngCodeRegion = %d , RngSymOffset = %d\e[0m\n", __LINE__, GetCurrentTime(), get_nid(),RngCodeRegion,RngSymOffset);
						}
					}
			...
			case CDMA_Alloc_IE:
                ...
                //printf("MS savedALie.ie_14.frameIndex = %d\n",savedALie.ie_14.frameIndex);
                //printf("MS savedALie.ie_14.rangCode = %d\n",savedALie.ie_14.rangCode);
                //printf("\e[1;35mMS(%d) CDMA_Alloc_IE relay_flag = %d\e[0m\n",get_nid(),LastSignalInfo.relay_flag);
				if (RngLastStatus == CodeSuccess || RngLastStatus == CodeWaitRSP)  // ranging code success
				{
                    ...
                    {
                        //printf("codeIndex: %d %d\n",(savedCode.rangingCodeIndex& 0xFF),(savedALie.ie_14.rangCode & 0xFF));
                    }
                    else if((savedCode.rangingSymbol  & 0xFF) != (savedALie.ie_14.rangSym & 0xFF))
                    {
                        //printf("rngsymbol: %d %d\n",(savedCode.rangingSymbol& 0x3FF),(savedALie.ie_14.rangSym & 0xFF));
                    }
                    else if((savedCode.rangingSubchannel  & 0x3F)  != (savedALie.ie_14.rangCh & 0x3F))
                    {
                        //printf("rngsubch : %d %d\n",(savedCode.rangingSubchannel& 0x3F),(savedALie.ie_14.rangCh & 0x3F));
                    }
                    else if(((savedCode.rangingFrameNumber & 0x0F)!= (savedALie.ie_14.frameIndex & 0x0F))&&
                            (!LastSignalInfo.relay_flag))
                    {
                        //printf("MS(%d) rngfnum  : %d %d\n",get_nid(),(savedCode.rangingFrameNumber & 0x0F),(savedALie.ie_14.frameIndex & 0x0F));
                    }
                    else if(((int)(savedCode.rangingFrameNumber & 0x0F) !=
                                ((int)((savedALie.ie_14.frameIndex-1) & 0x0F))) && (LastSignalInfo.relay_flag))
                    {
                        //printf("rngfnum  : %d %d\n",(savedCode.rangingFrameNumber & 0x0F),(savedALie.ie_14.frameIndex & 0x0F));
                    }
					else    // Send RNG-REQ at CDMA allocation slots
					{
                        ...
                        /*
                            printf("\e[1;37mTime:%llu MS[%d] timerSendRngReq->start at %llu, fSTime=%llu\e[0m\n", GetCurrentTime(), get_nid(), GetCurrentTime() + tick,LastSignalInfo.frameStartTime);
			...
			default:
				...
				{
                    //printf("MS(%d) receive ie_other (ie_cid=%d)\n",get_nid(),ie_cid);
                    memcpy(&savedULie, &tmpULie, sizeof(ULMAP_IE_u));
					MICRO_TO_TICK(tick, (TTG()+symbolsToPSs(_DLRelaySymbols))*PS()); // schedule sending time
 ...
 */
int mac802_16j_NT_PMPMS::procMOB_NBRADV(struct mgmt_msg *recvmm, int cid, int len)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::procMOB_NBRADV State: %x",get_nid(),State);
	...
	if (CfgCount != NeighborMRBSList->NBRADV_CfgCount)
	{
		...
		for (int i = 0;i < N_neighbors;i++)
		{
			...
			printf("### MS(%d):ChID   : %d\n\n", get_nid(), pNbrMRBS_NT->PreambleIndex);
			loopLen -= 1;
 ...
 */
int mac802_16j_NT_PMPMS::procRNGRSP(struct mgmt_msg *recvmm, int cid, int len)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::procRNGRSP State: %x",get_nid(),State);
	...
	while ((type = ifmm->getNextType()) != 0)
	{
		...
		switch (type) {
			...
			case 150: // only appear at replying CDMA Ranging Code
				...
				cdmaFlag = true;
				//printf("\e[1;36mMS(%d) rangAttr=%d cdmaFlag=%d\e[0m\n",get_nid(),rangAttr,cdmaFlag);
                break;
		}
	}
	...
	//printf("Time:%llu MS(%d)::%s() ( cdmaFlag = %d )\n", GetCurrentTime(), get_nid(), __func__,cdmaFlag);
	if (cdmaFlag == true)  // RNG-RSP because of CDMA
	{
        // Discard RNG-RSP not to this MS
		if ((savedCode.rangingCodeIndex   & 0xFF)  != ((rangAttr >> 8) & 0xFF))
        {
		    //printf("codeIndex: %d %d\n",(savedCode.rangingCodeIndex& 0xFF),((rangAttr >> 8) & 0xFF));
            return 0;
        }
	    else if((savedCode.rangingSymbol  & 0x3FF) != ((rangAttr >> 22) & 0x3FF))
        {
            //printf("rngsymbol: %d %d\n",(savedCode.rangingSymbol& 0x3FF),((rangAttr >> 22) & 0x3FF));
            return 0;
        }
        else if((savedCode.rangingSubchannel  & 0x3F)  != ((rangAttr >> 16) & 0x3F))
        {
            //printf("rngsubch : %d %d\n",(savedCode.rangingSubchannel& 0x3F),((rangAttr >> 16) & 0x3F));
            return 0;
        }
	    else if(((savedCode.rangingFrameNumber & 0xFF) != ((rangAttr & 0xFF))) && (!LastSignalInfo.relay_flag))
		{
            //printf("rngfnum  : %d %d\n",(savedCode.rangingFrameNumber & 0xFF),(rangAttr & 0xFF));
            return 0;
        }
        else if(((savedCode.rangingFrameNumber & 0xFF) != ((rangAttr-1) & 0xFF)) && (LastSignalInfo.relay_flag))
        {
            //printf("rngfnum  : %d %d\n",(savedCode.rangingFrameNumber & 0xFF),(rangAttr & 0xFF));
            return 0;
		}
		...
		else if (RngLastStatus == Continue) // 1:Continue
		{
			...
			RngTime = RngStart - 1;
			WaitForRangingOpportunity();
			//printf("\e[1;36mMS(%d) Period Ranging Continue! (ftimingadj = %d) TimingAdjust = %d\e[0m\n", get_nid(), ftimingadj, TimingAdjust);
			return 0;
		}
		else    // 3:Success
		{
			...
			if (RngCodeRegion == INITIAL_RANGING || RngCodeRegion == HANDOVER_RANGING)
			{
				//printf("\e[1;33mTime:%llu MS(%d) Initial/Handover Ranging Success!\e[0m\n", GetCurrentTime(), get_nid());
                // BS shall allocate a txop at CDMA_Allocation_IE
			}
			else if (RngCodeRegion == PERIOD_RANGING)
			{
				MILLI_TO_TICK(tick, PeriodRangingInterval * frameDuration_NT[frameDuCode()]);
...
int mac802_16j_NT_PMPMS::procSBCRSP(struct mgmt_msg *recvmm, int cid, int len)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::procSBCRSP State: %x",get_nid(),State);

	//printf("\nTime:%llu:mac802_16j_NT_PMPMS::%s()\n", GetCurrentTime(), __func__);
	return 1;
}

int mac802_16j_NT_PMPMS::procMOB_SCNRSP(struct mgmt_msg *recvmm, int cid, int len)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::procMOB_SCNRSP State: %x",get_nid(),State);
...
int mac802_16j_NT_PMPMS::procREGRSP(struct mgmt_msg *recvmm, int cid, int len)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::procREGRSP State: %x",get_nid(),State);
...
int mac802_16j_NT_PMPMS::procDSAREQ(struct mgmt_msg *recvmm, int cid, int len)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::procDSAREQ State: %x",get_nid(),State);
	...
	logRidvan(WARN,"ms nid:%d processed DSA-REQ data connection created cid:%d",get_nid(),fdatacid);
...
int mac802_16j_NT_PMPMS::procDSAACK(struct mgmt_msg *recvmm, int cid, int len)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::procDSAACK State: %x",get_nid(),State);
	...
	logRidvan(WARN,"ms processed DSA-ACK nid:%d cid:%d ",get_nid(),cid);
	delete ifmm;
	printf("\n\e[1;33m===============MS(%d)==IF=PROGRAM=RUN=TO=HERE=THAT'S=NETWORK=ENTRY=OK===============\e[0m\n\n", get_nid());

	return 1;
}

int mac802_16j_NT_PMPMS::procMOB_BSHORSP(struct mgmt_msg *recvmm, int cid, int len)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::procMOB_BSHORSP State: %x",get_nid(),State);
...
uint8_t mac802_16j_NT_PMPMS::selectDIUC(double recvSNR)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::selectDIUC State: %x",get_nid(),State);
 ...
 */
int mac802_16j_NT_PMPMS::SendRNGCODE()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::SendRNGCODE State: %x",get_nid(),State);
    ...
    {	//offset of perodic ranging code region
        codeIndex -= 43;
    }
	...
	// save to codeInfo
	codeInfo                = new RangCodeInfo(18*dupFactor, RngSymOffset, RngChOffset, rangMethod);
...
int mac802_16j_NT_PMPMS::SendRNGREQ()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::SendRNGREQ State: %x",get_nid(),State);
	...
	printf("\e[36mTime:%llu MS(%d):%s Receive SNR=%lf dB (from Node %d)and use diuc=%d(fec=%d)\e[0m\n", GetCurrentTime(), get_nid(),__func__, LastSignalInfo.SNR, LastSignalInfo.nid,myDIUC, DCDProfile[myDIUC].fec);
    ...
    //printf("MS len=%d toByte=%d burst->length=%d\n",len,thisLen,burst->length);
...
int mac802_16j_NT_PMPMS::SendREGREQ()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::SendREGREQ State: %x",get_nid(),State);
...
int mac802_16j_NT_PMPMS::SendSBCREQ()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::SendSBCREQ State: %x",get_nid(),State);
 ...
 */
int mac802_16j_NT_PMPMS::SendMOB_SCNREQ()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::SendMOB_SCNREQ State: %x",get_nid(),State);
	...
	// max = 5
	scanDuration = NeighborMRBSList->nbrBSs_Index.size() + NeighborMRBSList->nbrBSs_Full.size();
	if (scanDuration > 5)
	{
		scanDuration = 5;
	}
 ...
 */
int mac802_16j_NT_PMPMS::SendMOB_SCNREP()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::SendMOB_SCNREP State: %x",get_nid(),State);
 ...
 */
int mac802_16j_NT_PMPMS::SendMOB_MSHOREQ()
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::SendMOB_MSHOREQ State: %x",get_nid(),State);
	 ...
	 */
	for (iter = NeighborMRBSList->nbrBSs_Index.begin();iter != NeighborMRBSList->nbrBSs_Index.end();iter++)
	{
		if ((*iter)->targetHO == true)
			N_BS_Index++;
	}

	for (iter = NeighborMRBSList->nbrBSs_Full.begin();iter != NeighborMRBSList->nbrBSs_Full.end();iter++)
	{
		if ((*iter)->targetHO == true)
			N_BS_Full++;
	}
	...
	{
		ifmm->appendBitField( 8, i);
		ifmm->appendBitField( 8, NeighborMRBSList->nbrBSs_Index[i]->PreambleIndex);
		...
		if (((repMetric & 0x04) >> 2) == 1)
		{
			ifmm->appendBitField( 8, NeighborMRBSList->nbrBSs_Index[i]->RelativeDelay); // not implement
		}
	...
	for (int i = 0;i < N_BS_Full;i++)
	{
		...
		if (((repMetric & 0x04) >> 2) == 1)
		{
			ifmm->appendBitField( 8, NeighborMRBSList->nbrBSs_Full[i]->RelativeDelay);
		}
...
int mac802_16j_NT_PMPMS::SendMOB_HOIND(uint8_t HO_operation_mode)
{

	logRidvan(TRACE,"-->%d	mac802_16j_NT_PMPMS::SendMOB_HOIND State: %x",get_nid(),State);

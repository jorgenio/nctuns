...
NbrMRBS_NT::NbrMRBS_NT(int _nid, uint8_t *_addr)
{
	nid     = _nid;
...
NbrMRBS_NT::~NbrMRBS_NT()
{
	delete addr;
}

void NbrMRBS_NT::dump()
{
	printf("#########################\n");
	printf("# Index: %d\n", Index);
	printf("# Pream: %d\n", PreambleIndex);
...
NeighborMRBSs_NT::NeighborMRBSs_NT()
{
	NBRADV_CfgCount      = 1;
	ScanDuration         = 0;
	ReportMode           = 0;
...
NbrMRBS_NT *NeighborMRBSs_NT::getNbrbyBSID(uint8_t *_addr)
{
...
NbrMRBS_NT *NeighborMRBSs_NT::getNbrbyChID(uint8_t chID)
{
	vector<NbrMRBS_NT *>::iterator iter;
...
int NeighborMRBSs_NT::getNextScanChID()
{
	vector<NbrMRBS_NT *>::iterator iter;
	uint32_t Index_i    = 0;
	uint32_t Full_i     = 0;
...
bool NeighborMRBSs_NT::checkHOterms()
{
	vector<NbrMRBS_NT *>::iterator iter;
...
NbrMRBS_NT *NeighborMRBSs_NT::getTargetBS()  // Now we choose the best CINR to be the target BS
{
	vector<NbrMRBS_NT *>::iterator iter;
	NbrMRBS_NT *trgetBS  = NULL;
	int max_CINR    = 0;

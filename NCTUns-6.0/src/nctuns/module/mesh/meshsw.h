...
#ifndef __NCTUNS_MeshSW_h__
#define __NCTUNS_MeshSW_h__
...
#define	PT_INFRA  0x01
#define	PT_ADHOC  0x02
#define	PT_FIXED  0x03
...
class MeshSW : public NslObject {
 ...
 private: 
	...
	char			*fileName;
     ...
     public:
 	MeshSW(u_int32_t type, u_int32_t id, struct plist* pl, const char *name); 
 	~MeshSW();   
	...
	SwPort			*findport(u_char *mac_, swTbl *targetTbl,u_char* );
	int			findSwTbl(u_char *mac_, SwPort *port_, swTbl *targetTbl);

	int			addSwTbl(u_char *mac_, SwPort *port_,
						swTbl *targetTbl);

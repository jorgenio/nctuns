...
MODULE_GENERATOR(wphy);

extern SLIST_HEAD(headOfLink, con_list)          headOfWireless_;
extern RegTable					 RegTable_;

wphy::wphy(u_int32_t type, u_int32_t id, struct plist* pl, const char *name)
		: NslObject(type, id, pl, name)
{
	...
	RXThresh_ = -73.75;		// Desired Available Transmission Range (DATR)
...
int wphy::init() {
	...
	TransPower = pow(10, TransPower/10) * 1e-3;  // 15 dbm => pow(10, 15/10)*1e-3 Watt 
	...
	if ( (_log)&&(!strcasecmp(_log, "on")) ) {
		...
		BASE_OBJTYPE(type);
		type = POINTER_TO_MEMBER(wphy, log);
	...
	if( _linkfail && !strcmp(_linkfail, "on") ) {
		...
		if( linkfailFile == NULL ) {
			...
			typeStart = POINTER_TO_MEMBER(wphy, TurnOnLinkFailFlag);
			typeStop  = POINTER_TO_MEMBER(wphy, TurnOffLinkFailFlag);
	...
	*uPrecompute = 1;
	...
	SLIST_FOREACH(wi, &headOfWireless_, nextLoc)
	{
		...
		rxCSThresh_ = (double *)get_regvar(wi->obj->get_nid(), 
						...
						"CSThresh"); 
		if(rxCSThresh_ == NULL){
			/* The peer is not using wphy module. */
			printf("Node %d:: Not using wphy module\n", wi->obj->get_nid());
			continue;
		}
...
#ifdef WPHY_DEBUG
		printf("[WPHY %s]: recvPower %f, thresh %f\n", __func__, recvPower, *rxCSThresh_);
#endif
...
int wphy::recv(ePacket_ *pkt) {
	...
	/* get phy-information of incoming packet */
	...
	if (Pr < RXThresh_) {
		...
		/* packet error model */
		if ( BitError(wphyinfo->BER, p->pkt_getlen()) == 1){
			p->pkt_err_ = 1;  
		}
	}
...
int wphy::command(int argc, const char *argv[]) {
	return(NslObject::command(argc, argv));
}

int wphy::BitError(double BER_, int plen) {
	double	x,PER;
...
void wphy::TurnOnLinkFailFlag(Event_ *ep){
...
void wphy::TurnOffLinkFailFlag(Event_ *ep){

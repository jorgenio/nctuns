...
ChannelModel_80211a::ChannelModel_80211a(double frequency,double bandwidth,double symbolDuration, bool isEnabled)
{
...
ChannelModel_80211a::~ChannelModel_80211a()
{
	//printf("ChannelModel_80211a::~ChannelModel_80211a()\n");
}

double ChannelModel_80211a::computeBER(int codeType, double SNR)
{
	double ENR = SNRToENR(SNR, _dataRate[codeType]);
	double BER;

	/*printf("ChannelModel_80211a::computeBER()\n");
	printf("\tcodeType = %d\n", codeType);
	printf("\tSNR      = %lf\n", SNR);
	printf("\tdataRate = %lf\n", _dataRate[codeType]);
	printf("\tENR      = %lf dB\n", ENR);*/
...
double ChannelModel_80211a::computeBERbyENR(int codeType, double ENR)
{
	double BER;
...
void ChannelModel_80211a::makeBitError(char *input, int inputLen, double BER)
{
	...
	//printf("ChannelModel_80211a::makeBitError()\n");
	//printf("\tinputLen = %d, nerrors = %d, BER = %lf\n", inputLen,nerrors, BER);
...
//
#if 0
double ChannelModel_80211a::BPSK(double ENR)
{
...
double ChannelModel_80211a::QPSK(double ENR)
{
	return BPSK(ENR);
}
...
//   2000, p.223.
//
double ChannelModel_80211a::QAM(int M, double ENR)
{
	double BER;
	double d, logm;
	double sum = 0;
...
double ChannelModel_80211a::powerToSNR(double power)
{
...
//   p.97.
//
double ChannelModel_80211a::SNRToENR(double SNR, double dataRate)
{
	// Eb/N0 = (S/R) / (k*T)
	//       = (S/R) / (N/B)
	//       = (S/N) / (R/B)

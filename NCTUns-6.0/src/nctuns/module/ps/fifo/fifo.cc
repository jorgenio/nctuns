...
MODULE_GENERATOR(fifo);

fifo::fifo(u_int32_t type, u_int32_t id, struct plist* pl, const char *name)
...
int fifo::init() {
	...
	if ( (log_qlen_flag)&&(!strcasecmp(log_qlen_flag, "on")) ){
		...
		if ( (log_option)&&(!strcasecmp(log_option, "FullLog")) ){
			...
			BASE_OBJTYPE(type);
			type = POINTER_TO_MEMBER(fifo, log);
	...
	/* set upcall */
        upcall = (int (NslObject::*)(MBinder *))&fifo::intrq;
...
int fifo::command(int argc, const char *argv[]) {
	char			tmpBuf[10];

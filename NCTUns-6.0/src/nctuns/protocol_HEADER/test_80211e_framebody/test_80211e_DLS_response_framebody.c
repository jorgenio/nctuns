...
int main(int argc, char **argv) {
	...
	framebody_len = Calculate80211eDLSResponseFrameBodyLength(SupportedRates_Len, ExtendedSupportedRates_Len);
	...
	SetStatusCodeInto80211eDLSResponseFrameBody(framebody, &StatusCode);
	printf("\e[0;31;40mSet\e[m StatusCode = %d\n", StatusCode);

	u_char DestinationMACAddress[6] = {0x66, 0x55, 0x44, 0x33, 0x22, 0x11};
	SetDestinationMACAddressInto80211eDLSResponseFrameBody(framebody, (void *)DestinationMACAddress);
	...
	SetSourceMACAddressInto80211eDLSResponseFrameBody(framebody, (void *)SourceMACAddress);
	...
	SetCapabilityInfoInto80211eDLSResponseFrameBody(framebody, ESS, IBSS, CFPollable, CFPollRequest, Privacy, ShortPreamble, PBCC, ChannelAgility, SpectrumManagement, QoS, ShortSlotTime, APSD, DSSS_OFDM, DelayedBloackAck, ImmediateBlockAck);
	if(ESS)
	...
	SetSupportedRatesInto80211eDLSResponseFrameBody(framebody, SupportedRates_Len, (void *)SupportedRates);
	...
	SetExtendedSupportedRatesInto80211eDLSResponseFrameBody(framebody, SupportedRates_Len, ExtendedSupportedRates_Len, (void *)ExtendedSupportedRates);
	...
	GetStatusCodeFrom80211eDLSResponseFrameBody(framebody, &StatusCode);
	printf("\e[0;32;40mGet\e[m StatusCode = %d\n", StatusCode);

	GetDestinationMACAddressFrom80211eDLSResponseFrameBody(framebody, (void *)DestinationMACAddress);
	printf("\e[0;32;40mGet\e[m DestinationMACAddress = %x:%x:%x:%x:%x:%x\n", DestinationMACAddress[0], DestinationMACAddress[1],DestinationMACAddress[2],DestinationMACAddress[3],DestinationMACAddress[4],DestinationMACAddress[5]);	

	GetSourceMACAddressFrom80211eDLSResponseFrameBody(framebody, (void *)SourceMACAddress);
	printf("\e[0;32;40mGet\e[m SourceMACAddress = %x:%x:%x:%x:%x:%x\n", SourceMACAddress[0], SourceMACAddress[1],SourceMACAddress[2],SourceMACAddress[3],SourceMACAddress[4],SourceMACAddress[5]);	

	char *CapabilityInfo = GetCapabilityInfoFrom80211eDLSResponseFrameBody(framebody);
	...
	tmp_void_ptr = GetSupportedRatesFrom80211eDLSResponseFrameBody(framebody, &SupportedRates_Len);
	...
	tmp_void_ptr = GetExtendedSupportedRatesFrom80211eDLSResponseFrameBody(framebody, SupportedRates_Len, &ExtendedSupportedRates_Len);

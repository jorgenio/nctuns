...
int main(int argc, char **argv) {
	...
	timestamp = 10;
	SetTimestampInto80211eBeaconFrameBody(framebody, &timestamp); 
	printf("\e[0;31;40mSet\e[m timestamp = %llu\n", timestamp);

	BeaconInterval = 2;
	SetBeaconIntervalInto80211eBeaconFrameBody(framebody, &BeaconInterval);
	...
	SetCapabilityInfoInto80211eBeaconFrameBody(framebody, ESS, IBSS, CFPollable, CFPollRequest, Privacy, ShortPreamble, PBCC, ChannelAgility, SpectrumManagement, QoS, ShortSlotTime, APSD, DSSS_OFDM, DelayedBloackAck, ImmediateBlockAck);
	if(ESS)
	...
	SSID = 12;
	SetSSIDInto80211eBeaconFrameBody(framebody, SSID_Len, &SSID);
	...
	SetSupportedRatesInto80211eBeaconFrameBody(framebody, SSID_Len, SupportedRates_Len, (void *)SupportedRates);
	...
	HopPattern = 0x22;
	HopIndex = 0x33;
	SetFHParameterSetInto80211eBeaconFrameBody(framebody, optional_info_flag, SSID_Len, SupportedRates_Len, &DwellTime, HopSet, HopPattern, HopIndex);
	if(WithFH) {
	...
	CurrentChannel = 0x44;
	SetDSParameterSetInto80211eBeaconFrameBody(framebody, optional_info_flag, SSID_Len, SupportedRates_Len, CurrentChannel);
	if(WithDS) {
	...
	CFP_MaxDuration = 14;
	CFP_DurRemaining = 15;
	SetCFParameterSetInto80211eBeaconFrameBody(framebody, optional_info_flag, SSID_Len, SupportedRates_Len, CFP_Count, CFP_Period, &CFP_MaxDuration, &CFP_DurRemaining);
	if(WithCF) {
	...
	ATIM_Window = 16;
	SetIBSSParameterSetInto80211eBeaconFrameBody(framebody, optional_info_flag, SSID_Len, SupportedRates_Len, &ATIM_Window);	
	if(WithIBSS) {
	...
	SetTIMInto80211eBeaconFrameBody(framebody, optional_info_flag, SSID_Len, SupportedRates_Len, DTIM_Count, DTIM_Period, Bitmap_Control, PartialVirtualBitmap_Len, (void *)PartialVirtualBitmap);
	if(WithTIM) {
	...
	SetQBSSLoadInto80211eBeaconFrameBody(framebody, optional_info_flag, SSID_Len, SupportedRates_Len, PartialVirtualBitmap_Len, &StationCount, ChannelUtilization, &AvailableAdmissionCapacity);
	if(WithQBSS) {
	...
	AC_VO_ECW = 0xdd;
	AC_VO_TXOP_Limit = 44;            	
	SetEDCAParameterSetInto80211eBeaconFrameBody(framebody, optional_info_flag, SSID_Len, SupportedRates_Len, PartialVirtualBitmap_Len, QoSInfo_EDCAParameterSetUpdateCount, QoSInfo_QAck, QoSInfo_QueueRequest, QoSInfo_TXOPRequest, AC_BE_AIFSN, AC_BE_ACM, AC_BE_ECW, &AC_BE_TXOP_Limit, AC_BK_AIFSN, AC_BK_ACM, AC_BK_ECW, &AC_BK_TXOP_Limit, AC_VI_AIFSN, AC_VI_ACM, AC_VI_ECW, &AC_VI_TXOP_Limit, AC_VO_AIFSN, AC_VO_ACM, AC_VO_ECW, &AC_VO_TXOP_Limit);
	if(WithEDCA) {
	...
	SetQoSCapabilityInto80211eBeaconFrameBody(framebody, optional_info_flag, SSID_Len, SupportedRates_Len, PartialVirtualBitmap_Len, QoSInfo_EDCAParameterSetUpdateCount, QoSInfo_QAck, QoSInfo_QueueRequest, QoSInfo_TXOPRequest);	
	if(WithQoS) {
	...
	/*==Get=========================*/
	GetTimestampFrom80211eBeaconFrameBody(framebody, &timestamp);
	printf("\e[0;32;40mGet\e[m timestamp = %llu\n", timestamp);

	GetBeaconIntervalFrom80211eBeaconFrameBody(framebody, &BeaconInterval);
	printf("\e[0;32;40mGet\e[m BeaconInterval = %d\n", BeaconInterval);

	CapabilityInfo = GetCapabilityInfoFrom80211eBeaconFrameBody(framebody);
	...
	SSID_Len = 0;
	tmp_void_ptr = GetSSIDFrom80211eBeaconFrameBody(framebody, &SSID_Len);
	if(tmp_void_ptr != NULL && SSID_Len != 0) {
		bcopy(tmp_void_ptr, (void *)&SSID, SSID_Len);
		printf("\e[0;32;40mGet\e[m SSID = %d\n", SSID);
	}

	tmp_void_ptr = GetSupportedRatesFrom80211eBeaconFrameBody(framebody, SSID_Len, &SupportedRates_Len);
	...
	tmp_char_ptr = GetFHParameterSetFrom80211eBeaconFrameBody(framebody, SSID_Len, SupportedRates_Len);
	...
	tmp_char_ptr = GetDSParameterSetFrom80211eBeaconFrameBody(framebody, SSID_Len, SupportedRates_Len);
	if(tmp_char_ptr != NULL) {
		if(GetCurrentChannelFrom80211eDSParameterSet(tmp_char_ptr, &CurrentChannel) == true)
			printf("\e[0;32;40mGet\e[m CurrentChannel = %x\n", CurrentChannel);
	}

	tmp_char_ptr = GetCFParameterSetFrom80211eBeaconFrameBody(framebody, SSID_Len, SupportedRates_Len);
	if(tmp_char_ptr != NULL) {
	...
	tmp_char_ptr = GetIBSSParameterSetFrom80211eBeaconFrameBody(framebody, SSID_Len, SupportedRates_Len);
	if(tmp_char_ptr != NULL) { 
		if(GetATIMWindowFrom80211eIBSSParameterSet(tmp_char_ptr, &ATIM_Window) == true)  
			printf("\e[0;32;40mGet\e[m ATIM_Window = %d\n",ATIM_Window);
	}

	tmp_char_ptr = GetTIMFrom80211eBeaconFrameBody(framebody, SSID_Len, SupportedRates_Len);
	if(tmp_char_ptr != NULL) {
		if(GetCountFrom80211eTIM(tmp_char_ptr, &DTIM_Count) == true)
			printf("\e[0;32;40mGet\e[m DTIM_Count = %x\n", DTIM_Count);
		if(GetPeriodFrom80211eTIM(tmp_char_ptr, &DTIM_Period) == true)
			printf("\e[0;32;40mGet\e[m DTIM_Period = %x\n", DTIM_Period);
		if(GetBitmapCtrlFrom80211eTIM(tmp_char_ptr, &Bitmap_Control) == true)
			printf("\e[0;32;40mGet\e[m Bitmap_Control = %x\n", Bitmap_Control);
		PartialVirtualBitmap_Len = 0;
		tmp_void_ptr = (char *)GetPartialVirtualBitmapFrom80211eTIM(tmp_char_ptr, &PartialVirtualBitmap_Len);
	...
	tmp_char_ptr = GetQBSSLoadFrom80211eBeaconFrameBody(framebody, SSID_Len, SupportedRates_Len, PartialVirtualBitmap_Len);
	if(tmp_char_ptr != NULL) {
	...
	tmp_char_ptr = GetEDCAParameterSetFrom80211eBeaconFrameBody(framebody, SSID_Len, SupportedRates_Len, PartialVirtualBitmap_Len);
	if(tmp_char_ptr != NULL) {
	...
	tmp_char_ptr = GetQoSCapabilityFrom80211eBeaconFrameBody(framebody, SSID_Len, SupportedRates_Len, PartialVirtualBitmap_Len);
	if(tmp_char_ptr != NULL) {

...
int main(int argc, char **argv) {
	...
	SetDialogTokenInto80211eADDTSResponseFrameBody(framebody, DialogToken);
	printf("\e[0;31;40mSet\e[m DialogToken = %x\n", DialogToken);

	unsigned short StatusCode = 22;
	SetStatusCodeInto80211eADDTSResponseFrameBody(framebody, &StatusCode);
	printf("\e[0;31;40mSet\e[m StatusCode = %d\n", StatusCode);

	unsigned int Delay = 4455;
	SetTSDelayInto80211eADDTSResponseFrameBody(framebody, &Delay);
	printf("\e[0;31;40mSet\e[m Delay = %d\n", Delay);
	...
	if(TCLASFrameClassifierType == ClassifierType_0) {
		SetTCLASwithFrameClassifierType0Into80211eADDTSResponseFrameBody(framebody, optional_info_flag, UserPriority, ClassifierMask, (void *)SourceAddress, (void *)DestinationAddress, &Type);
		if(WithTCLAS) {
		...
		SetTCLASwithFrameClassifierType1ForIPv4Into80211eADDTSResponseFrameBody(framebody, optional_info_flag, UserPriority, ClassifierMask, Version, (void *)SourceIPAddressV4, (void *)DestinationIPAddressV4, &SourcePort, &DestinationPort, DSCP, Protocol);
		if(WithTCLAS) {
			printf("\e[0;31;40mSet\e[m UserPriority = %x\n", UserPriority);	
			printf("\e[0;31;40mSet\e[m ClassifierMask = %x\n", ClassifierMask);	
		...
		SetTCLASwithFrameClassifierType1ForIPv6Into80211eADDTSResponseFrameBody(framebody, optional_info_flag, UserPriority, ClassifierMask, Version, (void *)SourceIPAddressV6, (void *)DestinationIPAddressV6, &SourcePort, &DestinationPort, (void *)FlowLabel);
		if(WithTCLAS) {
			printf("\e[0;31;40mSet\e[m UserPriority = %x\n", UserPriority);	
			printf("\e[0;31;40mSet\e[m ClassifierMask = %x\n", ClassifierMask);	
			printf("\e[0;31;40mSet\e[m Version = %x\n", Version);	
		...
		SetTCLASwithFrameClassifierType2Into80211eADDTSResponseFrameBody(framebody, optional_info_flag, UserPriority, ClassifierMask, &TagType);
		if(WithTCLAS) {
			printf("\e[0;31;40mSet\e[m UserPriority = %x\n", UserPriority);	
			printf("\e[0;31;40mSet\e[m ClassifierMask = %x\n", ClassifierMask);	
	...
	SetTCLASProcessingInto80211eADDTSResponseFrameBody(framebody, optional_info_flag, Processing);
	...
	ServiceStartTime = 1234;
	unsigned int ServiceInterval = 4321;
	unsigned short SpecificationInterval = 666;
	SetScheduleInto80211eADDTSResponseFrameBody(framebody, optional_info_flag, Aggregation, TSID, Direction_Bit_1, Direction_Bit_2, &ServiceStartTime, &ServiceInterval, &SpecificationInterval);
	if(Aggregation)
	...
	GetDialogTokenFrom80211eADDTSResponseFrameBody(framebody, &DialogToken);
	printf("\e[0;32;40mGet\e[m DialogToken = %x\n", DialogToken);

	GetStatusCodeFrom80211eADDTSResponseFrameBody(framebody, &StatusCode);
	printf("\e[0;32;40mGet\e[m StatusCode = %d\n", StatusCode);

	char *TSDelay = GetTSDelayFrom80211eADDTSResponseFrameBody(framebody);
	if(TSDelay != NULL) {
		if(GetDelayFromTSDelay(TSDelay, &Delay))
			printf("\e[0;32;40mGet\e[m Delay = %d\n", Delay);
	}

	char *TSPEC = GetTSPECFrom80211eADDTSResponseFrameBody(framebody);	
	if(TSPEC != NULL) {
	...
	char *TCLAS = GetTCLASFrom80211eADDTSResponseFrameBody(framebody, &FrameClassifierType);
	if(TCLAS != NULL) {
	...
	if(FrameClassifierType == 0x00) {
		...
		TCLASProcessing = GetTCLASProcessingFrom80211eADDTSResponseFrameBody(framebody, ClassifierType);
	}
	else if(FrameClassifierType == 0x01 && TCLASFrameClassifierType == ClassifierType_1_IPv4) {
	...
	char *Schedule = NULL;
	ClassifierType = NoType;
	if(FrameClassifierType == 0x00) {
		ClassifierType = ClassifierType_0;
		Schedule = GetScheduleFrom80211eADDTSResponseFrameBody(framebody, ClassifierType);
	}
	else if(FrameClassifierType == 0x01 && TCLASFrameClassifierType == ClassifierType_1_IPv4) {
		ClassifierType = ClassifierType_1_IPv4;
		Schedule = GetScheduleFrom80211eADDTSResponseFrameBody(framebody, ClassifierType);

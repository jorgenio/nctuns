...
int main(int argc, char **argv) {
	...
	SetCapabilityInfoInto80211eReassociationRequestFrameBody(framebody, ESS, IBSS, CFPollable, CFPollRequest, Privacy, ShortPreamble, PBCC, ChannelAgility, SpectrumManagement, QoS, ShortSlotTime, APSD, DSSS_OFDM, DelayedBloackAck, ImmediateBlockAck);
	if(ESS)
	...
	ListenInterval = 2;
	SetListenIntervalInto80211eReassociationRequestFrameBody(framebody, &ListenInterval);
	...
	SetSupportedRatesInto80211eReassociationRequestFrameBody(framebody, SSID_Len, SupportedRates_Len, (void *)SupportedRates);
	...
	SetQoSCapabilityInto80211eReassociationRequestFrameBody(framebody, SSID_Len, SupportedRates_Len, QoSInfo_EDCAParameterSetUpdateCount, QoSInfo_QAck, QoSInfo_QueueRequest, QoSInfo_TXOPRequest);	
	...
	/*==Get=========================*/
	CapabilityInfo = GetCapabilityInfoFrom80211eReassociationRequestFrameBody(framebody);
	...
	GetListenIntervalFrom80211eReassociationRequestFrameBody(framebody, &ListenInterval);
	printf("\e[0;32;40mGet\e[m ListenInterval = %d\n", ListenInterval);

	GetCurrentAPAddressFrom80211eReassociationRequestFrameBody(framebody, (void *)CurrentAPAddress);
	printf("\e[0;32;40mGet\e[m CurrentAPAddress -> %x:%x:%x:%x:%x:%x\n", CurrentAPAddress[0], CurrentAPAddress[1], CurrentAPAddress[2], CurrentAPAddress[3], CurrentAPAddress[4], CurrentAPAddress[5]);

	SSID_Len = 0;
	tmp_void_ptr = GetSSIDFrom80211eReassociationRequestFrameBody(framebody, &SSID_Len);
	if(tmp_void_ptr != NULL && SSID_Len != 0) {
		bcopy(tmp_void_ptr, (void *)&SSID, SSID_Len);
		printf("\e[0;32;40mGet\e[m SSID = %d\n", SSID);
	}

	tmp_void_ptr = GetSupportedRatesFrom80211eReassociationRequestFrameBody(framebody, SSID_Len, &SupportedRates_Len);
	...
	tmp_char_ptr = GetQoSCapabilityFrom80211eReassociationRequestFrameBody(framebody, SSID_Len, SupportedRates_Len);

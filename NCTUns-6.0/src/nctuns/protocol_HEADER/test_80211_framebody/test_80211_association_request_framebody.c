...
int main(int argc, char **argv) {
	...
	CFPollRequest = false;
	Privacy = true;
	SetCapabilityInfoInto80211AssociationRequestFrameBody(framebody, ESS, IBSS, CFPollable, CFPollRequest, Privacy);
	if(ESS)
	...
	ListenInterval = 2;
	SetListenIntervalInto80211AssociationRequestFrameBody(framebody, &ListenInterval);
	...
	SetSupportedRatesInto80211AssociationRequestFrameBody(framebody, SSID_Len, SupportedRates_Len, (void *)SupportedRates);
	...
	/*==Get=========================*/
	CapabilityInfo = GetCapabilityInfoFrom80211AssociationRequestFrameBody(framebody);
	...
	GetListenIntervalFrom80211AssociationRequestFrameBody(framebody, &ListenInterval);
	printf("\e[0;32;40mGet\e[m ListenInterval = %d\n", ListenInterval);

	SSID_Len = 0;
	tmp_ptr = GetSSIDFrom80211AssociationRequestFrameBody(framebody, &SSID_Len);
	if(tmp_ptr != NULL && SSID_Len != 0) {
		bcopy(tmp_ptr, (void *)&SSID, SSID_Len);
		printf("\e[0;32;40mGet\e[m SSID = %d\n", SSID);
	}

	tmp_ptr = GetSupportedRatesFrom80211AssociationRequestFrameBody(framebody, SSID_Len, &SupportedRates_Len);

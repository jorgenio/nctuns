...
int main(int argc, char **argv) {
	...
	CFPollRequest = false;
	Privacy = true;
	SetCapabilityInfoInto80211AssociationResponseFrameBody(framebody, ESS, IBSS, CFPollable, CFPollRequest, Privacy);
	if(ESS)
	...
	StatusCode = 2;
	SetStatusCodeInto80211AssociationResponseFrameBody(framebody, &StatusCode);
	printf("\e[0;31;40mSet\e[m StatusCode = %d\n", StatusCode);

	AID = 12;
	SetAIDInto80211AssociationResponseFrameBody(framebody, &AID);
	...
	SetSupportedRatesInto80211AssociationResponseFrameBody(framebody, SupportedRates_Len, (void *)SupportedRates);
	...
	/*==Get=========================*/
	CapabilityInfo = GetCapabilityInfoFrom80211AssociationResponseFrameBody(framebody);
	...
	GetStatusCodeFrom80211AssociationResponseFrameBody(framebody, &StatusCode);
	printf("\e[0;32;40mGet\e[m StatusCode = %d\n", StatusCode);

	GetAIDFrom80211AssociationResponseFrameBody(framebody, &AID);
	printf("\e[0;32;40mGet\e[m AID = %d\n", AID);

	tmp_ptr = GetSupportedRatesFrom80211AssociationResponseFrameBody(framebody, &SupportedRates_Len);

...
int main(int argc, char **argv) {
	...
	SetSupportedRatesInto80211ProbeRequestFrameBody(framebody, SSID_Len, SupportedRates_Len, (void *)SupportedRates);
	...
	/*==Get=========================*/
	SSID_Len = 0;
	tmp_ptr = GetSSIDFrom80211ProbeRequestFrameBody(framebody, &SSID_Len);
	if(tmp_ptr != NULL && SSID_Len != 0) {
		bcopy(tmp_ptr, (void *)&SSID, SSID_Len);
		printf("\e[0;32;40mGet\e[m SSID = %d\n", SSID);
	}

	tmp_ptr = GetSupportedRatesFrom80211ProbeRequestFrameBody(framebody, SSID_Len, &SupportedRates_Len);

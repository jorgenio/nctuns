...
int brokenCarID = 0;
double agentSlowDownDistance = 999999; // Used for slow down the car when received broken car message
	...
	while(n > 0)
	{
		...
		if(p.type == RSUAGENT_REPORT_WARNING){
			...
			if(n > 0){
				...
				//printf("Agent %d receive msg from agent %d, distance %f\n", mynid, msg.nid, twoDistance);
				if(msg.seqNum > 0){
					int nidExists = 0;
						...
						if(msg.nid == msgSeq[i].nid)
						{
							...
							if(msg.seqNum > msgSeq[i].seqNum)
							{
								...
								if(msg.type == AGENT_CLIENT_IS_A_BROKEN_CAR){
									...
									if(ITS_AGENT_SLOW_DOWN){
										agentSlowDownDistance = Distance_BetweenTwoNode(msg.x, msg.y, CurrentPOS_x, CurrentPOS_y);
									}
									if(ITS_SHOW_MSG)
...
double DetermineAcceleration()
{
	...
	if(bfTime > 10)
		bfTime = 10;
	//else if(bfTime < 1)//Emergency deceleration!!
	//	return -4.5;
	//else if(bfTime < 1)
	//	bfTime=1;
	double acc;
	...
	// This agent will slow down its speed due to receiving the broken car message
	if((agentSlowDownDistance <= 250) && (agentSlowDownDistance > 100)){
		if(ITS_SHOW_MSG){
			printf("Agent %d receive a broken car message, distance %f, acceleration %f, speed %f\n", 
					mynid, agentSlowDownDistance, acc, CurrentVelocity);
		}
	}
	if((agentSlowDownDistance <= 100) && (agentSlowDownDistance > 40)){
		if(ITS_SHOW_MSG){
			printf("Agent %d receive a broken car message, distance %f, acceleration %f, speed %f\n", 
					mynid, agentSlowDownDistance, acc, CurrentVelocity);
		}

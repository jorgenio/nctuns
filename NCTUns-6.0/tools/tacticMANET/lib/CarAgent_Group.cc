...
int TeamID,TeamLeader;  //add by icchen 12.16.08   
...
//For papres, we move this into road.h, but after that, we should let it define in car agent.
...
char portNumStr[32];
double* nextRoadDirection;
double* curRoadDirection;
double* neighborRoadDirection;
int numDirection, numNeighborDirection;
double radius = 10.0; /* radius of a circle */
double cx, cy, cz; /* center of a circle */
double c_X, c_Y; /*correctness coordinate*/
double a, b, c, *CandidateDirection=NULL, endPOS_x, endPOS_y, FirstPointAfterTheTurn_x, FirstPointAfterTheTurn_y;
int RoadType;
int n;
int lookTo = 1;
int EnableLaneChange = 0; // second parameter of command line that user decide if enable LaneChange or not
int LaneChanging = 0; // Car is doing lane change
...
void init()
{
	...
	n = getProFileData(mynid, MaxVelocity, MaxAcceleration, MaxDeceleration);
	if(n == 0){
	...
	//To construct the whole map by using this function(Both new and old sturcture are constructed by using this API.).
	constructRoadMapofTheWholeField();
n = getTeamInfo(myTCPsockfd, mynid,TeamID,TeamLeader,1);
	n = getInitialNodePosition(mynid, CurrentPOS_x, CurrentPOS_y, CurrentPOS_z);
	CurrentPOS_z = 0;
	n = selfcorrectness(CurrentPOS_x, CurrentPOS_y, CorrectedPOS_x, CorrectedPOS_y, 1);
	if(n == 1)
	{
	...
	if(n == -1){
		...
		else
		{
			...
			if(distance <= 0.3){
				...
				//CollisionHandeling(mynid, NearestPrecedingCarID);
			}
...
void changeLaneOrNot(){
	...
	for(loop2=0; loop2<2; loop2++){
		free(neighborRoadDirection);
		neighborRoadDirection=NULL;
		if(lookTo == 1){
			n = getneighboringlaneinformation(CurrentPOS_x, CurrentPOS_y, RIGHT, numNeighborDirection, neighborRoadDirection, a, b, c, endPOS_x, endPOS_y, CacheOrNot);
		}
		else{
			n = getneighboringlaneinformation(CurrentPOS_x, CurrentPOS_y, LEFT, numNeighborDirection, neighborRoadDirection, a, b, c, endPOS_x, endPOS_y, CacheOrNot);
		}
		if(n == 1){
			double testAngle=0;
			testAngle = fabs(CurrentDirection-neighborRoadDirection[0]);
			if(fabs(testAngle - 180) <= 0.5){
				...
				tmpEvent.x = CurrentPOS_x + radius*sqrt(2)*cos(midDirectionArc);
				tmpEvent.y = CurrentPOS_y + radius*sqrt(2)*sin(midDirectionArc);
				tmpEvent.direction = midDirection;
				EventQueue.push_back(tmpEvent);

				//Second event point
				tmpEvent.x = CurrentPOS_x + 2*radius*sqrt(2)*cos(midDirectionArc);
				tmpEvent.y = CurrentPOS_y + 2*radius*sqrt(2)*sin(midDirectionArc);
				tmpEvent.direction = neighborRoadDirection[0];
				EventQueue.push_back(tmpEvent);
...
int main(int argc, char* argv[])
{
	...
	while(1)
	{
		...
		else if(n == 1)
		{
			...
			{
				double CorrectedPOS_x, CorrectedPOS_y;
				n = selfcorrectness(CurrentPOS_x, CurrentPOS_y, CorrectedPOS_x, CorrectedPOS_y, CacheOrNot);
				if(n == 1)
				{
					CurrentPOS_x = CorrectedPOS_x;
					CurrentPOS_y = CorrectedPOS_y;
				...
				n = setCurrentWaypoint(myTCPsockfd, mynid, CurrentPOS_x, CurrentPOS_y, CurrentPOS_z, 1);
				n = getcurrentroadinformation(CurrentPOS_x, CurrentPOS_y, numOfDirections, CandidateDirection, a, b, c, endPOS_x, endPOS_y, RoadType, CacheOrNot);
				if(n <= 0){
				...
				{
					//After the turn, if car is still on the previous road, make this car go to the final point of the previous road one more time.
					printf("Car %d Wake up in Node at time %f secs, position x= %f, y=%f, CurrentEvent x==%f, y==%f, diredtion==%f event direction==%f!!\n", mynid, (double)now.tv_sec, CurrentPOS_x, CurrentPOS_y, CurrentEvent.x, CurrentEvent.y, CurrentDirection, CurrentEvent.direction);
					ct=CT_ROAD_SEGMENT;
				...
				n = getnextintersectionorcurveinformation(CurrentPOS_x, CurrentPOS_y, CurrentDirection, ct, numOfDirections, CandidateDirection, a, b, c, CacheOrNot);
				if(n <= 0){
				...
				n = takeATurn(CurrentPOS_x, CurrentPOS_y, CurrentDirection, ExpectedDirection, numOfTurns, turningPOS_x, turningPOS_y, FirstPointAfterTheTurn_x, FirstPointAfterTheTurn_y, DirectionQueue, CacheOrNot);
				if(n <= 0){

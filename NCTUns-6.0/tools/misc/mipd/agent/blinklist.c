...
int
printBList(char *str, struct blinkList *plist)
{
...
newBListEntry(char *a, char *b, char *c, long int d)
{
...
struct blinkListEntry *
newBListEntry2(struct in_addr *a, struct in_addr *b, struct in_addr *c, long int d)
{
	struct blinkListEntry *pEntry;
...
int
addBEntry(struct blinkList *plist, char *a, char *b, char *c, long int d)
{
	struct blinkListEntry *p;
	...
	p = newBListEntry(a, b, c, d);
	if(p == NULL){
		fprintf(stderr, "[X] fail to newListEntry().\n");
		return -1;
	}else if( isEmptyBL(plist) ){ 
		plist->head = p;
		plist->tail = p;
		/* p->next = plist->head;
		plist->head = p;
...
int
addBEntry2(struct blinkList *plist, struct in_addr *a, struct in_addr *b, struct in_addr *c, long int d)
{
	struct blinkListEntry *p;
...
/* pending. */
int 
delBEntry(struct blinkList *plist, struct blinkListEntry *e, struct blinkListEntry *pre)
{

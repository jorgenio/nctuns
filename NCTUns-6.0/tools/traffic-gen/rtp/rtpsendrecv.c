...
/*
 * A simple rtp_send_receive application for NCTUns.
 * We receive & send both RTP and RTCP packets.
 */
...
double			trans_delay[40];	/* transmission delay time, in second */
	...
	if ( (argc > 5) && !strcmp(argv[5], "-t") ) {
		...
		int	i = 0;
        	...
        	if (fp) {
				...
				trans_delay[i] = atof(strtok(NULL, "\t\n "));
				if (trans_delay[i] < 0) {
					fprintf(stderr, "The delay time is illegal.\n");
                                        exit(1);
                                }
				fprintf(stderr, "trans_size[%d]=%d, trans_delay[%d]=%lf\n", i, trans_size[i], i, trans_delay[i]);
				i++;
			}
...
int main(int argc, char **argv){
	...
	while (rtp_interval > 0) {
		...
		if (trans_mode) {
			if (trans_size[index] < 0) {
				index = 0;
			}
			addtimestamp = ((int) trans_size[index] * (bits_per_sample / 8.) );

	                err = rtp_send(sid, marker, addtimestamp, codec_num, (int8 *)sendbuf, trans_size[index]);
	                if (err)
	                        err_handle(err);
			
			rtp_interval -= trans_delay[index];
			nexttime += trans_delay[index];
			index ++;
		}
		else {
		...
		marker = 0;	/* here we simply set marker = 0 since it is not the first packet of talkspurt */

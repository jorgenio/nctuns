...
int nbuf = 1024 * 1024 * 1024;		/* number of buffers to send in sinkmode */
...
struct timeval now;
unsigned long  lasttotal = 0, total=0;
int     ms=0, lastms=0;
double bandmega;
...
Usage: ttcp -t [-options] host [ < in ]\n\
       ttcp -r [-options > out]\n\
Common options:\n\
	-l ##	 length of bufs read from or written to network (default 1000)\n\
...
Options specific to -r:\n\
	...
	-w name  log per-second throuthput to file\n\
";	
...
void gotalarm(sig) {
	...
	bandmega = ((double)(nbytes-lasttotal)/(double)(ms-lastms));
	printf("%ld      %8d Kbyte/sec  ==>   %8.6lf Mbit/sec\n",
			now.tv_sec,
			(int)bandmega,
			(bandmega*8)/(1024)
	      );
	if(LOG){
		fprintf(file,
			"%ld      %8d Kbyte/sec  ==>   %8.6lf Mbit/sec\n",
			now.tv_sec,
			(int)bandmega,
			(bandmega*8)/(1024)
	               );
